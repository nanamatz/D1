...
- To clear a blind: spell words from `data/dictionary.txt` using hand letters, click `.play-btn`, wait for `.cashout` (Fee Settlement) or the play button to re-enable. Ante-1 Draft target is 300 — deliberately above a single opening word, so plan on a few plays. Then `.cashout .btn.cash` → shop (`.shop2`).
