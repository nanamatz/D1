...
export const BALANCE = {
  ...
  jokers: {
    ...
    longWordFan: { minLength: 5, chips: 38 },
    ...
    bookworm: { chipsPerWord: 7 },
    alliterationSticker: { mult: 7 },
    assonance: { repeatedVowels: 2, mult: 7 },
    porcelainCat: { mult: 9 },
    woodpecker: { chipsPerWood: 10 },
    letterLadderBadge: { chips: 57 },
    // Uncommon (§11.3)
    literaryJudge: { chips: 63 },
    rareEarth: { factor: 3.5 }, // ×Chips on Q·Z·X·J tiles
    glasswork: { multPerGlass: 7, lostPerBlind: 1 },
    voraciousReader: { chipsPerWord: 2 },
    classicist: { multPerFormal: 2 },
    streetCred: { chipsPerSlang: 10 },
    comboArtist: { mult: 8 },
    vowelMagnet: { factor: 1.63 },
    equilibrist: { chips: 50, mult: 5 },
    everydayHero: { minLength: 5, factor: 1.63 },
    formalInvitation: { gold: 3 },
    slangDictionary: { retriggers: 1 },
    oneVoice: { chips: 75 },
    civilTongue: { multPerBlind: 3 },
    uncensored: { chips: 100 },
    syllableScale: { difference: 1, chips: 44, mult: 5 },
    glassInsurance: { preventsPerBlind: 1 },
    growthRings: { chipsPerMult: 15 },
    materialSampler: { chipsPerMaterial: 19 },
    monomaterial: { mult: 10 },
    scrapDealer: { goldPerTile: 4 },
    lightTouch: { goldPerTile: 2 },
    heavyPress: { chipsPerTile: 19 },
    hollowPromise: { gold: 3 },
    doubleImpression: { retriggers: 1 },
    houseStyle: { chips: 63 },
    discardedDraft: { chipsPerTile: 4 },
    rewrite: { multPerDiscard: 3 },
    cleanCopy: { mult: 7 },
    fullDesk: { chipsPerHeldTile: 7 },
    clearDesk: { mult: 15 },
    lastSort: { chips: 100 },
    bagCounter: { tilesPerStep: 10, multPerStep: 3 },
    royaltyContract: { gold: 2 },
    bestsellerBand: { minLength: 7, gold: 3 },
    badReview: { gold: 3, multPenalty: 1, minMult: 1 },
    ...
    // Rare (§11.4)
    carteBlanche: { slots: 1, shopDiscount: 3 },
    hypocrite: { factor: 2.25 },
    rhymeChain: { factorPerMatch: 1.63 },
    outOfPrint: { chipsPerLetter: 32, multPerLetter: 4 },
    stargazer: { factorPerCard: 0.19 },
    fableHoard: { factorPerConsumable: 1.31 },
    anonymous: { factor: 2.88 },
    censorsBane: { factor: 2.88 },
    dadaist: { factor: 2.25 },
    interestGlutton: { multPerGold: 3 },
    acrosticPoet: { factor: 3.5 },
    alphabetPress: { factorPerPair: 1.31 },
    vowelChoir: { factorPerVowel: 1.25 },
    consonantChoir: { factorPerDuplicate: 1.44 },
    blackletterEngine: { retriggers: 1 },
    echoChamber: { retriggers: 1 },
    stoneTongue: { ignoredPerWord: 2 },
    glassCannon: { factorPerGlass: 1.31 },
    loadedLeadDice: { retriggers: 1 },
    woodblockPress: { factorPerGrowth: 0.06 },
    materialPrism: { factorPerMaterial: 1.31 },
    typeOrchestra: { factorPerFont: 1.38 },
    palindromist: { factor: 4.75 },
    straightShooter: { factor: 3.5 },
    vowelSymphony: { factor: 3.5 },
    longFormSerial: { freeLetters: 5, factorPerLetter: 1.31 },
    twinPeaks: { retriggers: 1 },
    threefoldSeal: { retriggers: 1 },
    handScholar: { factorPerNewHand: 0.25 },
    wordHunter: { factorPerNewWord: 0.04 },
    plagiarist: { factor: 4.75 },
    hotOffThePress: { minLength: 5, factor: 3.5 },
    nightOwl: { lowBag: 15, lowFactor: 3.5, emptyFactor: 6 },
    livingType: { chipsPerTile: 13 },
    typesettingMachine: { factorPerTile: 1.25 },
    synesthete: { factorPerCombo: 1.19 },
    royalWe: { factor: 3.5 },
    brokenSentence: { chips: 125, mult: 4 },
    holePunch: { factorPerGibberish: 0.19 },
    goldenType: { chips: 50 },
    deadlineAuction: { goldPerGrowth: 5, factorPerStep: 0.13 },
    termInsurance: { prevents: 4, chipsPerPrevent: 63 },
    exactingCritic: { factorPerRareLeft: 1.63 },
    // Legendary (§11.5)
