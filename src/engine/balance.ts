...
export const BALANCE = {
  ...
  scoreTypewriter: {
    ...
    keyPressMs: [0, 150, 120, 58 / 1.5, 51 / 1.5, 44 / 1.5] as const,
    keyPressFloorMs: 24 / 1.5,
  ...
  voucher: {
    ...
    bossRerollPrice: 10,
    /** Profile achievement thresholds for the 16 upgrade Vouchers (GDD §9.4). */
    unlockTargets: {
      novel: 50,
      theLaw: 50,
      fashionMagazine: 100,
  ...
  jokers: {
    ...
    recycling: { goldPerTile: 2 },
    beehiveTile: { wordLength: 6, baseChips: 66, chipsPerWord: 6 },
    ...
    comboArtist: { chipsFactor: 2 },
    vowelMagnet: { factor: 1.5 },
    equilibrist: { chips: 50, factor: 1.5, mult: 1.5 },
    ...
    heavyPress: { chipsPerTile: 40 },
    hollowPromise: { goldPerTile: 2, gold: 2 },
    doubleImpression: { retriggers: 1 },
    houseStyle: { chips: 150 },
    discardedDraft: { chipsPerTile: 7 },
    rewrite: { multPerDiscard: 3 },
    cleanCopy: { minDiscardsLeft: 4, mult: 10 },
    fullDesk: { chipsPerHeldTile: 25 },
    clearDesk: { mult: 15 },
    lastSort: { chips: 300 },
    bagCounter: { tilesPerStep: 10, multPerStep: 3 },
    royaltyContract: { gold: 2 },
    bestsellerBand: { minLength: 6, gold: 3 },
    badReview: { gold: 3 },
    sentenceOpener: { factor: 1.5 },
    verbEngine: { mult: 8 },
    modifierStack: { multPerPosTag: 5 },
    correctionMark: { mult: 13 },
    serial: { chipsPerMatch: 20 },
    ...
    alphabetPress: { factorPerLetter: 1.5, factorPerPair: 1.5 },
    vowelChoir: { factorPerVowel: 1.5 },
    ...
    golem: { multPerStone: 12 },
    temurah: { factor: 5 },
    alphabetPoet: { factor: 3.5 },
    iotaStroke: { factor: 2 },
    zombie: {},
    biochemistry: { baseFactor: 1, factorPerHand: 0.5, factorPerChain: 0.5 },
    ...
    misbound: { destroyDenominator: 1_000, factorPerSurvival: 0.5 },
  },
