...
export function GuidedIntro({
  ...
  const bodyKey = isPlayed('YELLOW') && (cur.key === 'frame' || cur.key === 'submit')
    ...
    : cur.key === 'palette' ? 'settings.paletteUnlock.confirmNotice' : `intro.step.${cur.key}.body`;
  const titleKey = cur.key === 'palette' ? 'settings.paletteUnlock.label' : `intro.step.${cur.key}.title`;
  ...
  return (
    <SpotlightBubble target={cur.selector} mascot="woodak" passthrough={!!gated}>
      <div className="intro-title">{t(titleKey)}</div>
      <p className="intro-body">{richText(t(bodyKey))}</p>
      <div className="intro-actions">
        <div className="intro-skip-progress">
          <button className="btn blue sm intro-skip" onClick={finish}>{t('intro.skip')}</button>
          <span className="intro-dots">{step + 1} / {INTRO_STEPS.length}</span>
        </div>
        {gated ? (
          <span className="intro-hint">{t(`intro.hint.${cur.key}`)}</span>
        ) : (
          <button className="btn sm intro-next" onClick={advance}>
