...
- **Run Info reference (changed 2026-08-01):** four tabs show Patterns, Letter Hands, Blinds, and Vouchers. Letter Hands lists all six hands in rank order with their condition, fixed Chips/Mult bonus, and whether gibberish qualifies. Its bonuses share the Patterns tab's one-line coloured `Chips × Mult` readout while retaining `+` on both additive axes.
