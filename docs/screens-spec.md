...
`wj.lang`, then a sanitized `en|ko|ja` hint only from a successfully initialized
...
breaks and omit terminal periods while preserving decimal points. Every numeric value is tagged for emphasis. English, Korean, and Japanese keys, interpolation
...
- Every game string through i18n (en/ko/ja); add later languages through the shared registry and a matching locale file.
