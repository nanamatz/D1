...
`wj.lang`, then a sanitized `en|ko|ja|zh-CN` hint only from a successfully initialized
...
- **Score Keyboard panel LED override (changed 2026-09-03):** all 101 physical keys have fixed six-colour LED slots. Only pressed keys bleed hard-edged pixel light into the gaps. Tier 5–6 deterministic sets include all six colours and give every pressed key its own beat-synchronised flame instead of the former central flame; Tier 6 flames are 25% larger and may emit one ember pixel. Tier 4 keeps smoke, Tier 5–6 keep smoke/POP, and idle/unpressed keys stay dark. During Tier 5–6 active and clear-held beats, the keyboard image's red/yellow/green top-panel indicators blink continuously with staggered phases; Tiers 0–4 leave them off. Panel LED/flame colour explicitly ignores Palette progress and `world-mono`. The former full-keyboard rainbow ring and Tier 6 jackpot sparks are retired. Reduced Motion freezes panel LEDs at low intensity and hides moving LED/per-key flame VFX while retaining the existing static pressed/Enter accent; Forced Colors hides decorative panel LED/flame. Visible/audible counts, sampled-key mapping, target and clear-repeat lifecycle, audio, engine state, and settle timing are unchanged.
- **Score Keyboard / 점수 키보드 (speed scope changed 2026-09-06):** the left margin owns one persistent non-interactive **1774×887, 2:1 landscape RGBA** vintage terminal keyboard with three empty raster wells. Raster + DOM keys rotate together 90° counter-clockwise into a vertical 1:2 dock: width `min(230px, 13vw, gutter − 16px, (100vh − 32px) / 2)`, height `width × 2`, centred, `contain`, never cropped, stretched, filtered, or made translucent. Its UI-only formula registry renders exactly 101 fully opaque labelled, disabled, untabbable keys: ESC/F1–F12, PRINT/SCROLL/BREAK, authentic no-Windows-key main block, ten nav/arrows, and 17-key numpad; A–Z appears exactly once and wide/tall keys retain ANSI-derived units. Every key is in the deterministic pool. Every `settleId` starts at Tier 0. Only a positive local score beat activates classification; its numerator is `max(0, Chips × Mult + flat score) + max(0, projectedScore − committedScore)`, with the visible projection delta snapshotted once per `settleId` and forced to zero under `previewHidden`. Divide by the positive finite target and classify Idle below 1× and Tiers 1–6 at `[1, 1.1, 1.2, 1.3, 1.4, 1.5]`, retaining the maximum peak. Decreases, zero/gold/stat/invalid beats and sentence assist alone cannot lower or falsely raise it; committed/earlier scores and later BUILD/LAND are excluded directly. Eligible beats use that current peak with their semantic primary (letter/Stone→Space, global→Enter, untargeted Emoji Tile→stable F-key, boss→Break, Tag→Tab, Pouch→Space, fallback→Enter) plus deterministic fill to 3/5/8/12/16/20 visible keys and 1/2/3/4/5/6 sounds, primary first. Each sound resolves that same visible key through a locally bundled 32-WAV pool: registry index i maps to sample (i mod 32) + 1, unknown ids use main Enter, and play failure falls back to the matching existing synthesized recipe without eager Loading-screen preload. Normal settle completion idles. An actual clear with a non-zero peak, including a sentence-assisted peak after a positive local beat, starts an immediate Enter-first synthetic cycle after settle completion and repeats during BUILD/LAND, final count-up, verdict, and the live Fee Settlement row/total animation at the fixed 460ms beat × Tier 1–6 factors `[2, 1.75, 1.5, 1.25, 1, 1]`, independent of selected speed; active key motion/audio and the target cue also retain current 1× timing. Each cycle uses Enter's mapped WAV first at the existing `deskEnter` accent gain, then each deterministic fill key's mapped WAV at the ordinary `deskKeycap` gain for the remaining audible slots, local tier body/key shake, Tier-4 smoke, and Tiers-5–6 smoke/flame/POP; Tier 6 held ambient uses low .72, high .92, 18px glow, and 400ms speed, while tier light remains between beats. Collect's transition to Shop, terminal modal, next blind, unmount, or Reduced Motion ON cancels all Score Keyboard repeat timers/audio/VFX synchronously. Temporary settle/tile/Emoji VFX still finish on their ordinary timelines. Tier-0 clear, loss, a true local-zero sentence-only clear, and nonclear paths never loop. The exact target-cross event is still one logical once-per-blind Enter strike; celebrations intentionally reuse Enter's mapped WAV at the `deskEnter` accent gain. Reduced Motion preserves a static peak/Enter accent and the existing target-cross audio exception only. Committed count-up, right-side encounters, `world-mono`, global shake, and settle timing/gate otherwise remain unchanged.
...
breaks and omit terminal periods while preserving decimal points. Every numeric value is tagged for emphasis. English, Korean, Japanese, and Simplified Chinese keys, interpolation
...
- Every game string through i18n (en/ko/ja/zh-CN); add later languages through the shared registry and a matching locale file.
...
- **Score Keyboard jackpot/smoke follow-up (changed 2026-09-03):** red/yellow/green indicators use deterministic random-looking order plus rapid hard-edged jackpot pulses. Each overlay centre aligns with its painted indicator while the light casts wider horizontal pixel rays. Tier 4–6 pressed keys emit clearly visible smoke and Tier 5–6 pressed keys emit clearly visible flames with deterministic 0.95–1.05 per-key variation; Tier 6 applies ×1.35 smoke and ×1.25 flame base multipliers. Tier 5 adds seven distributed small chassis-smoke particles, while Tier 6 uses all twelve at a larger dark-gray size. No single smoke overlay is rendered.
