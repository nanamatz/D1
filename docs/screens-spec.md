...
- **Score Typewriter (unlabeled authored caps revised 2026-08-29):** the left viewport margin is reserved for one persistent, non-interactive 320×1760 transparent RGBA pixel typewriter. It has no desk, props, background, frame, halo, or paper; the raster remains a body-only chassis with no baked keycaps. Its dock starts at 3vh, occupies 94vh and the safe lane width, and never crosses the board. The invisible key positioning plane contains 27 empty, disabled, untabbable native buttons authored individually with x/y position, scale, and tilt. They imply loose curved 10/9/8 mechanical banks without row/group wrappers, visible legends, a grid, border, or panel background. Idle caps are round/slightly squashed navy-charcoal mechanisms with muted metal rims, a one-pixel highlight, and hard bottom/right shadows. A deterministic 32-bit beat hash creates the unique permutation; configured `0.35` jitter creates uneven normalized gaps without `Math.random()`, engine RNG, or run state. Only the chosen cap moves inward 2–3px, scales slightly, collapses its shadow, and gains a subtle dull-gold face. Tier 1–5 retain 3/5/8/12/16 visible caps and 1/2/3/4/5 matching sounds, with every release inside the snapshotted 1×/2×/4× beat. Reduced Motion cancels key SFX and all key/carriage/body/shake/smoke/flame/POP motion and statically accents only the selected caps; there is no rectangular keywell cue. Chassis filtering/layering, carriage, smoke, Meltdown flame/POP, DING, tier formula, sentence BUILD/LAND, target crossing, committed count-up, settle timing/gate, Laboratory reuse, right-side encounters, and `world-mono` behavior remain unchanged.
...
- **Guided intro discard (changed 2026-08-27):** the lesson is four steps (frame → discard → build → submit). In step 2/4, only the first spare after the six protected YELLOW physical tiles accepts right-click mark/unmark; the red Discard button enables only while that target is marked, and only the ordinary successful discard/refill advances. Staging, other marks, sort, drag, and Play remain locked. Step 3/4 restores the exact Y→E→L→L→O→W ID lock and disables discard. A prior real discard, missing spare, or zero budget falls through to build without a duplicate charge or softlock; Skip clears any uncommitted tutorial mark and releases the lock.
