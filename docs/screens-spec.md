...
**Score Keyboard panel LEDs (changed 2026-09-03):** Tier 4–5 active and clear-hold beats blink the three red/yellow/green indicator LEDs painted in the keyboard image, using a deterministic random-looking staggered order that changes each beat/cycle. The full-keyboard rainbow ring and jackpot sparks are retired. Reduced Motion freezes these LEDs at low intensity; Forced Colors hides them.
...
- **Text-selection suppression (changed 2026-09-09):** dragging anywhere in the game UI must not produce the browser's blue text-selection highlight. Apply this at `body` so body-portalled overlays follow the same rule, while text inputs, textareas, and editable content explicitly retain native text selection.
...
- **Back button (changed 2026-09-09)**: full-width gold bar pinned at the bottom of every sub-screen. Localized copy contains only the action label; one shared decorative pixel left-arrow icon precedes it without entering the accessible name.
...
- **Native checkboxes (changed 2026-09-10):** retain native checkbox semantics but use one shared square, hard-edged pixel/CRT frame, gold check, visible keyboard focus, disabled treatment, and Forced Colors fallback across every surface.
...
- **Card tooltip — universal, no exceptions.** Hovering **any** interactive object anywhere opens an anchored tooltip: Emoji Tiles, consumables, letter tiles, vouchers, packs, Starting Pouches, Records, Tags, **and every item revealed inside an opened pack**. Contents: name, full base effect text, and — for scaling or decaying Emoji Tiles — the **current live value** (e.g. "currently ×1.5" or Folding Manuscript's current hand size). All main and supplemental tooltip copy is centre-aligned. Pouch Tag likewise appends `(currently +N Chips)`, calculated from `blind.bag` during an active/prepared blind and the complete `run.bag` in the Shop. Enhancement names remain coloured footer tags. With one supplemental definition it prefers the left. With two or three, exactly one definition folds into the main effect plate and the rest prefer the left; the fold priority is **material → font → edition**. Exception: when one description references exactly three Emoji Tile editions, all three edition definitions stay together; if another definition also exists, only the highest-priority non-edition definition may fold. Letter tiles use this for material/font/edition; Emoji Tiles use it for non-base editions. Any effect description that names a tile font automatically adds that font's canonical effect tooltip. A Record tooltip describes that level's added penalty and marks the ladder as cumulative. One shared component; a surface that renders objects without wiring it is a bug, not a gap. **Every tooltip is portalled to `document.body`, uses the highest UI layer, and is vertically contained by the physical viewport. It must never be clipped, covered, or trapped by a product panel's overflow/stacking context. Supplements move to the right when only that side fits and stack inside the main card when neither side fits; up/down direction does not otherwise change packing.** *(Changed 2026-09-06: locale-safe containment supersedes the absolute left-only placement; current-value and fold rules are unchanged.)*
  ...
  - **Emoji Tile performance copy and aggregate trigger (changed 2026-09-09):** when the separate live-value row already includes the base value, the effect plate omits that duplicate performance and contains only the rule/decay explanation. This applies to Beehive Tile, Dulling Pencil, Drying Ink, Folding Manuscript, Shuriken, and Word Hunter across every locale and shared tooltip surface. Non-dynamic standalone values may still use their own first line. Pouch Tag, Scrap Dealer, and Out of Print each score and present their complete displayed current Chips/Mult in one Emoji Tile beat, never one beat per contributing pouch step, Brass tile, or extinct letter.
...
- **Description face (changed 2026-07-31):** main and supplemental description plates use bundled Jost 700 with Noto Sans KR 700 fallback, compact tracking, and restrained cyan/magenta separation. This keeps Korean readable offline while giving effect copy the printed arcade tone of the reference.
  Semantic highlight phrases stay atomic while space permits: `+2 Mult` / `+2 배수`, money values, rarity names, and card-kind names move to the next line as a whole, but may break as a final containment fallback.
  - **Text wrapping (changed 2026-09-06):** descriptive copy keeps words intact where possible (`word-break: keep-all`) and uses `overflow-wrap: anywhere` as the shared last-resort safety net. No translated token may escape its surface or viewport. Pouch selection, clicked-open pouch descriptions, controls, and mascot/tutorial dialogue inherit the same rule. Speech bubbles size to their copy inside the existing surface cap and use greedy normal wrapping, never balanced wrapping.
  ...
  - **Actual-object motion ownership (changed 2026-09-04):** reusable Emoji Tile, Starting Pouch, and Record renderers own one idle/tilt/sheen layer by default, matching Voucher cards. A Shop/Pack or other composite that already moves the complete product passes `motion=false` to its child renderer; nested motion owners are forbidden. Static preload, decorative sticker, and transient VFX art remain raw.
  ...
  - **Enhancement stack (containment changed 2026-09-06):** material, font, and edition names render as distinct coloured tags stacked beneath the letter-card tooltip. Their normal-weight definitions follow the shared count-aware layout: one prefers a side card; with two or three, the highest-priority definition folds into the main plate and the remainder stay together, moving right or inside when the viewport requires it.
...
- **Incomplete card-family pages (changed 2026-09-10):** on desktop, the last Constellation or Gambler page distributes its remaining cards evenly across the full five-card row instead of leaving trailing empty columns. Narrow layouts retain their responsive grid.
...
or separate visible Skip button. Its bundled
...
completely separate from `src/ui/audio.ts`, settings/storage, Music/SFX levels,
their mute checkboxes, and MUSIC/SOUND or colour gates. It attempts ungestured playback
...
700ms text-only fallback; a late image stays suppressed. Preparing black frames ignore and consume input without unlocking gameplay audio. Once the ready logo or fallback text is actually visible, the complete viewport becomes a focusable `role=button` surface with one localized action name; any ordinary non-shortcut keydown, primary left/touch/pen pointer press, or accessibility synthetic click immediately mounts Loading. The accepted event prevents default and propagation, so it cannot click through or trigger App's delegated `buttonPress`/`audio.unlock`. Timer, input, and StrictMode races share one pre-locked finish path, while unmount clears the automatic timer and stops any scheduled/running ident cue. The existing 3850ms/2240ms/700ms automatic completions remain when no input occurs. Preparation otherwise shows only
the black backdrop, without text, animation, or fake progress. The complete
ident and logo always render in their original colours with `filter: none`;
they never read profile unlocks or use `#unlock-chroma`. (changed 2026-08-31)
...
Before the first interactive frame, language resolves as valid machine-wide
`wj.lang`, then a sanitized `en|ko|ja|zh-CN|zh-TW|pt-BR|de|es-ES|fr-FR|ru-RU|pl-PL|tr-TR` hint only from a successfully initialized
packaged Windows Steam launch, then English. The detected hint is never persisted,
so only the player's language control changes `wj.lang`; web, direct-EXE, failed
Steam initialization, and unsupported Steam locales keep the saved choice or
English. This also localizes the first Steam ownership prompt without a flash.
(Japanese and Simplified Chinese localization added 2026-09-06; Traditional Chinese, Brazilian Portuguese, German, Spanish (Spain), French, Russian, Polish, and Turkish added 2026-09-07.)

Title treatment (our own logotype), production buttons: **Play · Options · Collection · Quit**. Development builds add one **Laboratory** button between Collection and Quit. The title uses a stepped, diagonal hard-shadow extrusion and a slight perspective pitch so it reads as chunky 3D pixel art rather than flat type. It floats through eight discrete vertical/tilt steps while the red exclamation mark periodically punches through a short three-frame accent; both animations are Main-Menu-only and stop under either reduced-motion path. Behind the menu, a low-resolution phosphor grid and slow full-screen scan beam animate beneath the global CRT pass; both reduced-motion paths freeze the beam while retaining the static texture. Quit shows on all builds: it attempts `window.close()` (works in a desktop shell / script-opened window) and always swaps the menu for a full-screen farewell ("Thanks for playing!"), so a normal browser tab that can't self-close still ends cleanly (2026-07-22). The buttons keep this vertical order inside one dark CRT panel, with individual block shadows and palette-mapped semantic colors. Laboratory is a non-gameplay, single replaceable review surface for the latest completed player-facing implementation or change that cannot be inspected immediately in Collection or Run Info; internal-only refactors do not qualify, new qualifying work replaces old content, and automatic tests own retired preview regressions. It reuses production components/data and never reads or writes engine, RNG, run, discovery, profile, settings, or storage state. The current surface reuses production `ScoreTypewriter` with deterministic UI-only Tier 1–5 controls and Replay so the updated key counts, 1.5×-faster cadence, physical depression, LEDs, smoke, and flames can all be checked while resizing the viewport. It keeps the real full-motion, both Reduced Motion paths, responsive, accessible, and Forced Colors behavior. The shared 1440×988 shell, CRT treatment, Back, and Escape remain. The former coffee-cup, money-ledger, money-total-motion, hidden-pattern, and word-score-transfer previews are retired without changing their real behavior or automatic regressions. The bottom-left profile control is a compact dark card with a `Profile` header and inset active-profile-name button. The bottom-right language control mirrors that card, displays the currently selected language (`English`, `한국어`, `日本語`, `简体中文`, `繁體中文`, `Português (Brasil)`, `Deutsch`, `Español (España)`, `Français`, `Русский`, `Polski`, or `Türkçe`) without an icon, keeps every active label on one centred line and widens or clips its compact card instead of wrapping, and opens a centred three-column language-selection modal (button no-wrap rule changed 2026-09-08). This Main Menu modal is the sole language-change surface. Supported languages populate the grid in registry order, collapse to one column on narrow viewports, visibly mark the current choice, and persist a changed choice before reloading the game shell; choosing the current language does not reload. The modal retains a centred full-width Back action; the current choices are English, Korean, Japanese, Simplified Chinese, Traditional Chinese, Brazilian Portuguese, German, Spanish (Spain), French, Russian, Polish, and Turkish, with later languages added one registry row at a time. Collection badges a `!` when new words/jokers were discovered last run. (Language flow changed 2026-09-08; Laboratory content changed 2026-09-07.)

**Profiles (changed 2026-09-05).** The profile button opens a three-slot screen matching the reference layout. P1 exists on first boot with the editable default name `P1`; P2/P3 begin empty. A slot tab only previews that slot and never changes the active profile; creating a slot also leaves the active profile unchanged. A selected non-active created slot becomes active only when the blue **Load Profile** button is pressed, which reloads the shell against that slot. The active slot instead shows a disabled **Current Profile** button and its smaller red Delete Profile button is also disabled. Selecting an empty slot shows a name field, Empty panel, Create Profile, disabled red Delete Profile, and Back. The empty and populated dashboards use the same panel dimensions and column proportions. Creating with a blank name uses `P2`/`P3`; every created profile can be renamed later. A created slot shows the editable name, overall progress, four progress rows (word Collection, presentation, Starting Pouches, Record wins), lifetime wins, compact balance telemetry, profile status/load, Delete Profile, Reveal All, and Back. Profile vocabulary/register titles no longer exist; legacy `equippedRegisterTitle` data is ignored and omitted by normalized future writes without a save-key or version change. Balance telemetry counts only non-custom-seeded completed runs and shows recorded run count, win rate, and the Chapter with the most losses; per-Chapter loss counts remain profile-scoped in `wj.lifetime`. The nine progress save keys and profile metadata are isolated per slot while language, settings, and sort order remain machine-wide. Existing flat saves are P1. Delete Profile is available only for a non-active created slot and uses the same-button two-press warning: the first press only shows an inline destructive warning, and the second press deletes exactly the previewed slot. On the first Reveal All press, no effect is applied and a persistent per-profile warning explains the consequence. A later press fills every implemented locked/undiscovered registry in that profile and disables its Challenges; no other profile is changed. Once used, the Reveal All button is replaced by **Challenges disabled**. A profile that earns every implemented word, presentation, Starting Pouch, Record, and upgraded-voucher unlock naturally instead shows **Your world is complete**. The Reveal All button uses the same muted teal background as the main-menu profile control and the same compact 34px height as Delete Profile. The compact applied marker makes all dictionary entries read as discovered without creating fake per-word play counts or scores.

### 2.2 New Run
**Shared New Run modal (changed 2026-09-04).** One compact `720×700px` border-box framed modal owns a
fixed tab row, internally scrolling content viewport, a primary-action row that
is `82px` normally and `116px` only while the seeded-run input is visible, with
a centred action capped at `340px`, an always-reserved optional-note row, and
one full-width Back row. Tab changes and conditional Challenge notes never
move the action or Back controls. At short heights and 200% zoom, only the
content viewport scrolls; the shared controls remain reachable. Production has
exactly **New Run · Continue** tabs. Development builds alone append
**Challenges** by conditionally creating its tab and panel; production does not
place either in the DOM or accessibility tree, and the Challenge Start
implementation chunk is absent from the production JavaScript bundle.
Continue reuses the New Run Pouch/Record art contract exactly: `140×140px` and
`88×88px` on desktop, then `92×92px` and `68×68px` at `≤620px`.
...
corrupt save is **discarded, not migrated**. Continue uses the real tooltipped
Pouch and Record objects. Its white summary plate shows Chapter and current
blind, committed score/target (never projected or final score), Fee/돈, best word
and score (or an em dash), and seed; the Record also shows its effect and
cumulative note. A saved Challenge shows its localized name only in the reserved
note row, including in production, and restores its known `challengeId` plus seed; an unknown
...
shared tooltips/cursor reaction plus both localized effects. It does not repeat
the selected Challenge name or description above the loadout, keeping the compact
shared shell free of a content scrollbar. **Start Challenge**
...
reason. The tab strip is one `tablist`; every available control exposes `tab`,
`aria-selected`, and `aria-controls`, with a matching `tabpanel` id. The
Challenges start surface is DEV-only; Challenge progress, active runs, Run Info,
completion, and Continue compatibility remain supported in production.
...
- **Score Keyboard panel LED override (changed 2026-09-03):** all 101 physical keys have fixed six-colour LED slots. Only pressed keys bleed hard-edged pixel light into the gaps. Tier 4–5 deterministic sets include all six colours and give every pressed key its own beat-synchronised flame instead of the former central flame; Tier 5 flames are 25% larger and may emit one ember pixel. Tier 3 keeps smoke, Tier 4–5 keep smoke/POP, and idle/unpressed keys stay dark. During Tier 4–5 active and clear-held beats, the keyboard image's red/yellow/green top-panel indicators blink continuously with staggered phases; Tiers 0–3 leave them off. Panel LED/flame colour explicitly ignores Palette progress and `world-mono`. The former full-keyboard rainbow ring and jackpot sparks are retired. Reduced Motion freezes panel LEDs at low intensity and hides moving LED/per-key flame VFX while retaining the existing static pressed/Enter accent; Forced Colors hides decorative panel LED/flame. The five Tiers use 5/10/12/16/20 visible keys and 3/4/5/6/7 sounds; sampled-key mapping, target and clear-repeat lifecycle, engine state, and settle timing remain unchanged.
- **Score Keyboard / 점수 키보드 (feedback speed/counts changed 2026-09-07):** the left margin owns one persistent non-interactive **1774×887, 2:1 landscape RGBA** vintage terminal keyboard with three empty raster wells. Raster + DOM keys rotate together 90° counter-clockwise into a vertical 1:2 dock: width `min(230px, 13vw, gutter − 16px, (100vh − 32px) / 2)`, height `width × 2`, centred, `contain`, never cropped, stretched, filtered, or made translucent. Its UI-only formula registry renders exactly 101 fully opaque labelled, disabled, untabbable keys: ESC/F1–F12, PRINT/SCROLL/BREAK, authentic no-Windows-key main block, ten nav/arrows, and 17-key numpad; A–Z appears exactly once and wide/tall keys retain ANSI-derived units. Every key is in the deterministic pool. Every `settleId` starts at Tier 0. Only a positive local score beat activates classification; its numerator is `max(0, Chips × Mult + flat score) + max(0, projectedScore − committedScore)`, with the visible projection delta snapshotted once per `settleId` and forced to zero under `previewHidden`. Divide by the positive finite target and classify Idle below 1× and Tiers 1–5 at `[1, 1.2, 1.3, 1.4, 1.5]`, retaining the maximum peak. Decreases, zero/gold/stat/invalid beats and sentence assist alone cannot lower or falsely raise it; committed/earlier scores and later BUILD/LAND are excluded directly. Eligible beats use that current peak with their semantic primary (letter/Stone→Space, global→Enter, untargeted Emoji Tile→stable F-key, boss→Break, Tag→Tab, Pouch→Space, fallback→Enter) plus deterministic fill to 5/10/12/16/20 visible keys and 3/4/5/6/7 sounds, primary first. Each sound resolves that same visible key through a locally bundled 32-WAV pool: registry index i maps to sample (i mod 32) + 1, unknown ids use main Enter, and play failure falls back to the matching existing synthesized recipe without eager Loading-screen preload. Normal settle completion idles. An actual clear with a non-zero peak, including a sentence-assisted peak after a positive local beat, starts an immediate Enter-first synthetic cycle after settle completion and repeats during BUILD/LAND, final count-up, verdict, and the live Fee Settlement row/total animation at the 1.5×-faster fixed `418ms / 1.5` (≈279ms) beat × Tier 1–5 factors `[2, 1.5, 1.25, 1, 1]`, independent of selected speed; active Tier 1–5 key motion/audio uses that same 1.5×-faster cadence and a visible 4px/.86 physical depression, while the target cue retains its current timing. Each cycle uses Enter's mapped WAV first at the existing `deskEnter` accent gain, then each deterministic fill key's mapped WAV at the ordinary `deskKeycap` gain for the remaining audible slots, local tier body/key shake, Tier-3 smoke, and Tiers-4–5 smoke/flame/POP; Held tiers retain their 1.5×-faster LED/smoke cadence, while the former gold chassis border and glow are removed. Collect's transition to Shop, terminal modal, next blind, unmount, or Reduced Motion ON cancels all Score Keyboard repeat timers/audio/VFX synchronously. Temporary settle/tile/Emoji VFX still finish on their ordinary timelines. Tier-0 clear, loss, a true local-zero sentence-only clear, and nonclear paths never loop. The exact target-cross event is still one logical once-per-blind Enter strike; celebrations intentionally reuse Enter's mapped WAV at the `deskEnter` accent gain. Reduced Motion preserves a static peak/Enter accent and the existing target-cross audio exception only. Committed count-up, right-side encounters, `world-mono`, global shake, and settle timing/gate otherwise remain unchanged.
- **Score Keyboard low-tier press override (changed 2026-09-10):** Tier 1 uses a `150ms` press, Tier 2 uses `120ms`, and both hold a deeper `6px`/`.78` down state. Tiers 3–5 retain `4px`/`.86` and their existing fast timings. This supersedes the Tier 1–2 portion of the general motion sentence above; LED/audio counts, repeat scheduling, and settle timing are unchanged.
- **Sidebar** (final-settlement axis display clarified 2026-09-06): play, Shop, and Blind Select use the same fixed `264px` top-badge footprint; with the common round panel and `484px` score/control allocation, every mode fills the same `956px` inner rail without a mode-specific bottom gap. The stage banner shows Draft/Revision/Deadline + ❄target + reward `$$$`; on the current Deadline only, the boss emblem is hoverable and keyboard-focusable and opens the shared portalled localized boss-name/full-description tooltip, including runtime interpolation such as Dead Letter's forbidden letter; selected Record icon/name with its cumulative penalties in the tooltip; committed-only round score; large ScoreBox; the always-reserved 52px status slot above that box ordinarily shows vertically centred, single-line/ellipsized selected-tile status (Word-Hand name / "not a word" / suit name); the enlarged Word-Hand label keeps a safe line box and top clearance under Forced Colors. ScoreBox precedence is finalized sentence bonus fixed at 0 × 0 → active word settle → staged Word-Hand preview → 0. Ordinary word-settle beats replay their Chips and Mult values; final sentence settlement keeps both axes at 0 × 0 so the committed round score is never replayed there. A staged Hand shows its exact bonus Chips and Mult, including ×1; debuffed or handless previews show 0 × 0, while undiscovered secret Hand names remain `???` without hiding their numbers. On a positive finite ordinary word result, full motion waits for the authoritative settle-complete signal, then that same slot shows the committed gain at the round score's `var(--readout-size)` and line-height 1 without a plus sign while Chips × Mult reads 0 × 0. The gain and round hold for 500ms at every game speed; then the gain decreases to zero while the round increases over the existing 700ms tween. The remainder is `clamp(committedScore − displayedRound, 0, committedScore − committedBefore)`, formatted by `formatScore` and decorative/`aria-hidden`. Invalid, zero, negative, and non-finite gains are absent and add no hold. Reduced Motion keeps the same static 500ms post-settle hold at 0 × 0, then removes the gain and snaps the round to committed without tweening. On a full-motion clear with a real sentence bonus, the word transfer runs alone from t500–1200 while sentence bonus remains absent and the ScoreBox stays 0 × 0; BUILD runs t1200–1900 and LAND runs t1900–2600 with the axes held at 0 × 0, and only then does the verdict beat begin. A positive no-bonus clear may publish its final score at t500 because the existing resolution wait covers the transfer; zero or non-finite gain remains immediate. Reduced Motion at or through the initial hold publishes the collapsed result at t500. The empty slot remains so the ScoreBox never moves. The UI-only hold timer is keyed by `settleId` and cancelled on a new settle, mode change, or unmount. No event, audio, shake, input, engine/RNG/save state, settle-complete signal, or settle-duration change is added. The sidebar also contains **Run Info** and **Options** buttons; phase & discard counts; `$` fee; **Chapter N/8**.
- **Final sentence-pattern callout and retained provenance (score-panel layout changed 2026-09-04):** when the last word's authoritative settle-complete signal lets BUILD publish a finalized real pattern, show one central localized `Lv.N · [pattern icon] pattern name` title using the existing 1.7-second Not Allowed animation family. Never show it for live projection, an ordinary word settle, or an Unison-only final. Reduced Motion uses a readable static/fade version and this callout never changes settle timing. Reuse the round panel's reserved live-pattern line for the finalized headline: pattern-only `Lv.N · [icon] Pattern`, Unison-only `[suit] Unison +Chips|×Mult`, Mixed-only `[synergy] ×factor Chips`, or pattern plus the winning style after ` ·`; defensive dual-style data chooses Unison. The complete headline uses the style semantic colour, but visible names/axes/values and a full accessible label remain authoritative, and long English combinations wrap without clipping. Do not add a duplicate score-box stamp. The score panel keeps the 52px status/transfer slot empty through BUILD so the ScoreBox does not move. Supplemental Word-Hand-footprint rows are only Modifier → grouped Effects → Pouch, with every non-neutral effect axis visible. The snapshot remains through BUILD, LAND, final count-up, verdict, and live Fee Settlement. Collect clears it in the Shop transition and clears any still-running notice before Shop paints. Zero or one supplemental row keeps score/controls at 188/296px; two rows use 211/273px; three rows use 234/250px, always totaling 484px.
- **POS chip palette (changed 2026-08-30):** all ten POS ids have unique neutral tokens and unique final colours. Palette ownership stays noun/interjection = BLUE, three verb subtypes = RED, adjective/adverb/article = YELLOW, conjunction/preposition = GREEN. Localized labels plus active-solid/alternative-dashed state remain authoritative under monochrome and forced colours.
- **Run Info reference (changed 2026-09-07):** four tabs show Patterns, Word Hands, Blinds, and Vouchers. When `challengeId` is present, the localized current Challenge occupies a full-width CSS-only gold pixel banner between the modal header and tabs: 56px minimum height, 10×16px padding, 12px bottom gap, 3px frame, inset edge, 4px hard shadow, 22px text, safe wrapping, and corner ornaments. With no Challenge the element is absent and reserves no space. Pattern and Word Hand rows share the exact level-color ladder and CSS variable: level 1 white, 2–3 yellow, 4–5 orange, 6–8 green, 9–12 blue, 13–16 purple, and 17+ red. They also share a rightmost boxed run-use count after their Chips/Mult readout, rendered as a bare numeral with no unit suffix. Patterns begins with nine public rows; Object Complement (5형식), Ditransitive (수여문), and Complex (복문) are absent from layout, DOM, accessibility, and tooltip output until the current run's first successful authoritative highest-pattern activation. Compound remains public. A revealed row persists through later pattern loss and save/Continue, resets on a new run, and initially retains a run-use count of 0 because activation does not replace finalization counting. Every visible pattern row is keyboard-focusable and touch-pinnable. Its shared body-portalled tooltip is the sole pointer-interactive tooltip, preserving constrained-viewport scrolling, and keeps the existing description above a compact tray made from the exact §5.2 uppercase words only, with no Example/예시 heading, divider, role/POS labels, or clause captions. Each ordinary word box uses its selected POS tag colour for its fill and border; special grammar tokens such as ISNT use a neutral marker tone rather than an invented POS colour. Every example, including Compound and Complex, lists all boxes in one uninterrupted left-to-right row with no arrows or row breaks; constrained viewports scroll that row horizontally instead of wrapping it. One localized word-sequence summary serves assistive technology, and pattern-only viewport-aware vertical placement keeps the tooltip inside the physical viewport. Hidden patterns construct none of this content before discovery. Word Hands lists all nine hands in rank order with their fixed Chips/Mult bonus and run-use count; their conditions and gibberish eligibility use the same shared portalled row tooltip as visible sentence-pattern descriptions, not inline secondary copy. Type Economy, Vowelless, and Grand Palindrome mask both tooltip name and condition as `???` until first completion in that profile. The selected-tile sidebar status uses the same mask, while the successful settle stamp performs the reveal. The nine-row list scrolls inside the modal at short viewports so the modal itself can keep overflow visible for portalled tooltips. A hand with Mult uses the one-line coloured `+Chips ×Mult` readout: Chips keeps its additive `+`, while Mult drops the `+` after `×`.
- **Guided intro action target (Shop finale changed 2026-09-10):** the lesson has five steps (frame → discard → build → submit → Palette Convenience), with the same real discard, exact physical-ID lock, resume/fallthrough, Skip cleanup, and settle-complete rules. Its three gated steps reuse one dynamic `.tutorial-action-target`: the unmarked spare is the sole right-click target; once marked, enabled Discard becomes the sole left-click target; build moves that left-click target through the exact Y→E→L→L→O→W IDs; submit moves it to enabled Play. YELLOW settlement releases the board lock and defers the fifth step. After Fee Settlement is collected and the first Shop is visible, the in-run Options opens directly to Settings > Game and spotlights the real `.palette-unlock-row .btn`; Next or Skip returns to the Shop without activating it, then queues `shopFirstVisit`. `SpotlightBubble` retains its `document.body` portal, per-RAF target lookup/live viewport rect tracking, and 8px padding. The spotlight box has no positional transition, so it follows a newly assigned or animated target on the next paint without hover instead of chasing stale coordinates. Fine mouse pointers alone show a neutral, `aria-hidden`, pointer-inert 40×40 CSS mouse just outside and within 48px of the live target rect. Coarse pointers show none; Reduced Motion makes it static; its contrast remains readable in `world-mono`.
- **Run Info pattern-example spacing (changed 2026-09-04):** the uninterrupted example row fills the tooltip width, centres its word-box group, and distributes the boxes evenly; constrained viewports retain the existing horizontal overflow rather than wrapping.
- **Collection Record-sticker hit priority (changed 2026-09-10):** the upper-right sticker, not its parent Emoji Tile, owns the Record tooltip. Its focusable hover target suppresses the parent tooltip throughout the overlap, so only one tooltip can open there.
- **No cash-out button** — the blind auto-settles (GDD §7.2).
- **Score/money formatting and action semantics (changed 2026-09-04):** main ScoreBox Chips/Mult and relevant money totals/deltas round once to integers, comma-format rounded absolute values below 10,000,000, use the largest `M/B/T/Qa/Qi` unit from 10,000,000 with at most one decimal truncated toward zero, retain compact scientific notation from `1e21`, and show an em dash for non-finite values; semantic effect popups retain their own precision. ScoreBox formats each axis once, preserves the exact independent values, and classifies the visible string only: ≤7 characters at 52px, 8–9 at 38px, and ≥10 at 30px. The centred inner numeral is single-line/clipped, while the outer box remains overflow-visible for score-pop VFX; localized accessible labels announce both exact strings. Buy/Redeem are blue, Use/Use Now green, and Sell red. Shop Buy/Redeem/Open retain 44px height and an 82px minimum width. A selected held consumable remains pointer-active so a second click closes its reversible action menu; required single-choice settings remain non-nullable.
- **Money total motion (changed 2026-09-04):** every distinct displayed balance change immediately updates the one formatted total and restarts a transform-only animation on its nested text span. Gains hop upward for 480ms and peak at `translateY(-4px)` with slight squash/stretch; losses mirror at `translateY(4px)`. Initial mount and unchanged values do not animate. Both Reduced Motion paths remove travel, Forced Colors keeps the same readable total, and no clone or layout shift is introduced. Existing signed popups remain. A Shop ledger-suppressed transaction still moves the total once by its atomic net direction while only the ordinary net popup is suppressed.
- **Will Score Keyboard guard (changed 2026-09-09):** the once-per-blind target cue is enabled only by the authoritative post-boss `projectedScore ≥ target` state. Will remains one final Chips ×0.5 and Mult ×0.5 boss beat. Its submission suppresses intermediate Score Keyboard beats and starts one beat from the final post-Will axes at the beginning of the existing final tally hold, without changing settle timing.
- **Score-panel spacing (status alignment changed 2026-09-06):** a visible internal gap separates the ScoreBox from modifier/effects/pouch rows. The always-reserved status/transfer slot is exactly 52px high, vertically centres every variant, and safely clips its one-line label without clipping glyph tops. The 188/211/234px score-panel heights, paired 296/273/250px controls, and 484px rail total remain fixed.
...
- **Stereotype Plate blocking (changed 2026-09-02):** its current threshold is the longest valid word already played this Chapter. Sidebar, Blind Select, Boss Intro, and Run Info reuse the headless current-minimum helper to show the exact numeric limit, or no current restriction at 0; Collection keeps generic effect copy. A staged hand shorter than that threshold uses the dedicated red/Mult blocked status and disables Play; this is an illegal submission, not the ordinary warning or zero-score boss treatment above. Forced Colors preserves the distinction.
...
The completed blind does not navigate away. The persistent sidebar resets round score, Chips, Mult, hands and discards to zero and changes its blind badge to a large, centred marquee **SHOP** sign with a restrained idle glow/bulb cycle around all four outer edges. The SHOP badge stretches to the full inner width of the shop sidebar without changing its footprint, hit box, or baseline; reduced motion leaves the full perimeter visible and freezes its blink. The score reset is immediate: the consumed settle log/id, finalized-score fields, and sentence-bonus fields are cleared before the shop's first frame, and the count-up hook snaps to zero instead of replaying the last blind's score. Owned Emoji Tiles, consumables and the pouch remain in place, but the pouch contents/count immediately switch to the complete permanent `run.bag`; the completed blind's partial `blind.bag` is never shown in the shop. A lower shop panel rises in this order: **Next Blind → Reroll → items for sale → voucher → packs**. The two rail buttons remain equal-width and equal-height inside the fixed 168px column in every locale; localized copy wraps to show its complete text, using reduced horizontal padding and last-resort word breaking instead of widening into the sale layer or showing an ellipsis. The three selling panels—items, voucher, and packs—centre their live offers with equal spacing and symmetric balance. Sold entries leave layout entirely, and every affected panel immediately recentres its remaining offers. Selecting an offer raises its complete product/price/action interaction owner by 59px (15px base + 44px action height), so the visible object and its hit-test geometry remain identical, and attaches its primary Buy/Redeem/Open button 8px directly below the actual foreground object. That primary control keeps its original 82px minimum width even though other blue/red controls use a larger locale-stable width. When available, **Use now** is the exception: it appears vertically centred 12px outside the product's right edge while Buy remains centred below. Buy and Use-now actions have a minimum 44px hit height. Card selection and its revealed action are separate buttons, so Redeem cannot toggle the card instead of firing. Tooltips are body-portalled per §0 and therefore do not depend on sale-panel overflow or stacking. (full-label wrapping changed 2026-09-09; rail overflow corrected 2026-09-09; action width corrected 2026-09-03; marquee changed 2026-08-20)
...
- **Money-gaining Fable ledger (changed 2026-09-04):** successful Shop **Use now** for registry effects `doubleGold` and `jokerSellGold` presents ordered nonzero signed text: `−actual price` first, then `+full payout`. It never substitutes the net delta. The committed purchase/use remains atomic, the shared result vignette opens once with the un-netted payout, and zero beats are omitted. Full motion gives the two beats distinct sequential triggers above the vignette; Reduced Motion keeps both readable in cost-then-payout DOM order without travel. Rapid transactions queue. The persistent Sidebar is the sole Shop balance and owns the single portalled ledger/live region only in Shop mode; leaving Shop immediately hides and clears active or queued ledger presentation. Ordinary money changes retain the shared popup outside Shop. Audio, engine/RNG/save state, affordability, consumption, stock removal, and counters are unchanged.
...
- **Voucher redemption (changed 2026-07-30):** Redeem shreds the voucher
  ...
  immediately.
- **Shop mascot:** **삐약이 (Piyak)**, a pixel-art **tuxedo cat proprietor**, stays bottom-aligned below the Next Blind/Reroll controls in the existing 168px left rail (behind-the-counter feel), which has no separate balance panel, and never overlaps the slots. Idle animation (single-sprite CSS breathe) + a content-sized, `--fs-lg` speech bubble showing one random `voice.piyak.welcome.*` line per shop entry through `voicedKeys`, per UI_DESIGN §6. Purchase/reroll reactions are a later layer. Runtime art: `src/ui/assets/piyak.png`.
...
During word settlement, contribution beats run at **600ms per tile/effect at 1× speed**, then divide by the selected speed. Before beat zero, every submitted tile moves as one rigid row through one **fixed 650ms** desk slam at every game speed, independent of word length; contact remains at 40%. One shared contact tick starts all local material-family bursts together and may fire at most one settings-scaled screen shake and one `submitThock`; there is no tile stagger or final-tile transform. Reduced motion keeps the fixed 700ms settle with only a static contact highlight. Order is whole-word stamps → each played tile with its own enhancements and tile-triggered Emoji effects → global Emoji Tile list → held tiles frozen in visible play-time order → future consumable hooks → global/boss beats. The current tile lifts and bounces on every contribution. Its own base/material/font/edition values appear above it; an Emoji-authored tile effect only animates the target letter tile and shows its value once, below the firing Emoji Tile. The settle-complete signal derives from this same cadence. Speed is snapshotted per submission only for ordered score-event cadence and never replays an in-flight submission. Impact, the 650ms final tally hold when score beats exist, tile/source VFX, audio/shake lifetime and intensity, the fixed 1.5×-faster Tier 1–5 Score Keyboard cadence, score transfer, BUILD/LAND, notices, verdicts, modals, and transitions stay fixed; feedback still begins with its source event. Reduced Motion ON switches outstanding work to the fixed 700ms branch, and OFF applies from the next submission. A chance-bearing Lead Plate material beat keeps a **600ms real-time minimum** at every speed so its rolled value remains visible, and the same duration source delays completion accordingly (group slam superseded per-tile impact on 2026-08-23).

**Owned-list edition order (changed 2026-09-03):** within the global Emoji Tile list, each physical owner resolves its Gray/Violet/Rainbow scoring edition before its intrinsic effect and growth/decay beats, then advances left-to-right. Base/White add no scoring-edition beat. Tile-bound Emoji effects remain in the earlier played-tile phase.

Every definition left after the shared count-aware fold is an independent framed card kept in one supplemental stack, including material/font references, enhancement definitions, and Gibberish definitions. The stack prefers the left, uses the right when needed, and folds inside the main tooltip when neither side fits. Its title uses the same bold white cyan/magenta-separated treatment as the main card. Every tooltip effect that needs a free consumable or Emoji Tile slot uses the shared capacity phrase “Requires available space” / “공간이 있어야 합니다” / “空き枠が必要” rather than naming the slot type.
...
breaks and omit terminal periods while preserving decimal points. Every numeric value is tagged for emphasis. English, Korean, Japanese, Simplified Chinese, Traditional Chinese, Brazilian Portuguese, German, Spanish (Spain), French, Russian, Polish, and Turkish keys, interpolation
...
Challenges 2–6. Word discoveries, secret Word Hands, and Emoji
...
The recap card shell remains static. Each physical Emoji Tile, Voucher, Pouch,
and Record owns its actual-object motion layer; both members of Record+Pouch and
Challenge pairs move independently, without a pair-level or nested tilt owner.
...
snapshot is ready. Natural immediate ChromaticReveal remains unchanged. Settings'
Palette Convenience grant uses one separate immediate aggregate reveal only after
its recap baseline is durable; its synthetic ids and Reveal All bulk grants never
enter the recap.
...
| **Enhanced Tiles** | two paged views using the shared `< · page · >` footer: page 1 Materials = 9 tile faces (base + 8 enhanced, including Wood); page 2 Fonts = 5 Futura variants | both pages render large pixel-art tile swatches; a concrete Wood tile's main tooltip combines letter Chips with its live Wood bonus, while generic Collection/reference copy states only +10 growth per play and never fabricates a current +15; Font tooltips show each seal effect from `balance.ts` `fontEffects` (changed 2026-08-31) |
...
Tabs — unified to three pages (changed 2026-09-03):
- **Game** (Palette Guide setting added 2026-09-09): game speed (ordered 0.5/1/2/4 — ordered score-event spacing only; 1 is default) · screenshake slider · reduced motion toggle (mirrors `prefers-reduced-motion`, user-overridable) · hint highlight color-blind-safe palette toggle · **Tutorial Tips** toggle for the guided introduction and first-encounter tutorial popups · separate default-On **Palette Guide** toggle for ordinary Palette reminders · current-profile **Palette Convenience / 팔레트 편의성** action. Palette Guide Off immediately cancels an active or queued reminder and suppresses later reminders without changing Tutorial Tips. The first Palette Convenience press only shows an inline `role=alert` irreversible warning; a second press of that same button grants exactly the four colors plus MUSIC/SOUND, never mascots or other progress. It persists the profile grant, durably absorbs the synthetic ids into any active/saved-run recap baseline, then emits one atomic aggregate event containing only actually added definitions. The global `ChromaticReveal` applies presentation and one fanfare once, reuses its 2.6-second/click-dismiss/Reduced-Motion grammar, and shows only `The world has been restored` / `세상을 되찾았습니다` / `世界を取り戻しました`; six individual word reveals are forbidden. Its `document.body` portal is layered above the in-run pause overlay and below tooltips so confirmation cannot expire unseen behind Settings. Tab change, Back, or unmount disarms it. Completed profiles show a disabled completed state.
- **Video** (Graphics merged 2026-09-03): resolution · fullscreen · UI scale · **CRT effect on/off · scanline intensity slider · CRT bloom on/off**, in that order (see UI_DESIGN §"Surface language"). In the Windows desktop app, Resolution sets windowed **content size** to one of four equal choices—960×600, 1280×800, 1600×900, or 1920×1080—with no Recommended marker. A preset is disabled when its decorated outer window would exceed the current display work area; manual resizing reads `Custom — W×H / 사용자 지정 — W×H`. Selecting an available preset while maximized restores the window, applies the content size, and centres it on that display. Fullscreen disables Resolution and the native Electron state—including F11—is authoritative; leaving fullscreen restores the preceding windowed bounds. The browser instead shows disabled `Automatic — browser controlled / 자동 — 브라우저에서 관리`, never calls `resizeTo`, and keeps the existing `fullscreenElement`/ESC synchronization. `window-state.json`, not `wj.settings`, remains the desktop resolution authority. CRT Off hides scanlines, vignette, and bloom; Bloom is independently switchable. The fixed 1440×988 board keeps continuous responsive viewport-fit plus UI scale. There is no physical display-mode change, render-resolution setting, monitor select, VSync, or integer-scale option. The broader Reveal All action belongs exclusively to the selected profile screen (§2.1); the Game tab's limited current-profile Palette grant is its only Settings exception.
- **Audio** (layout/copy clarified 2026-08-30): the note reads exactly `Audio / 오디오`. Independent Music and SFX sliders with value badges drive the **live gameplay Web Audio mixer** (`src/ui/audio.ts`, feature-01 B). Both rows share `label | slider | value | mute`: desktop `minmax(72px,1fr) minmax(140px,160px) 44px 76px` with 12px gaps, and ≤480px `64px minmax(96px,1fr) 36px 68px` with 8px gaps. Each native checkbox visibly reads `Mute / 음소거` and has a bus-qualified accessible name. There is no public Master control. Slider edits never unmute, while unmuting restores the retained slider value. Effective output is the matching `MUSIC`/`SOUND` Palette bus gate AND not-muted, then group volume × recipe gain. New-format mute booleans ignore stale Master. Otherwise valid positive Master migrates once through `round(master × group / 100)`; Master 0 preserves both group values and mutes both. Legacy defaults 80/70/80 become Music 56/SFX 64, both unmuted, and the next normalized write omits Master. SFX and two loop-safe synthesized BGM contexts (menu/run) remain shipped; Shop keeps the run loop muffled and Deadline uses it unchanged. The gameplay context still unlocks on the first gesture. The preceding ident cue (§2.0) remains a separate fixed-output startup context obeying none of these controls or gates. The Audio tab has no preview controls.
All 15 adjustable row settings use the shared tooltip on row hover, native-control focus, and touch pin; Escape, outside tap, tab change, and unmount close it without consuming the setting action. The separate Palette action explains its irreversible effect through the two-press warning above.

**Palette reminder (setting changed 2026-09-09).** Once the guided YELLOW lesson is complete, the first normal settle of each blind may queue one non-blocking hint for a still-locked required Palette word, provided the separate default-On Palette Guide setting is On and the board is input-stable. A required unlock on that submission or a blind-ending settle skips that blind. Visible copy uses one short localized `{word}` template, such as `“RED”로 해금`, instead of six per-word explanatory sentences. The six color/audio candidates circulate in registry order without repeating until every still-locked candidate has appeared; this memory is session-only and uses no game RNG. Pause, Run Info, Boss Intro, blind-entry effects, any coach-mark/chromatic reveal/modal, and non-playing phases suppress it. Render the complete notification as a keyboard-activatable button inside a polite status live region, portalled to `document.body` at the physical viewport's top-right. Its width shrink-wraps the localized text and close mark, with only a viewport-safe maximum that wraps unusually long translations. It slides in from the right for 240ms, holds for six seconds, then exits right for 180ms; hover/focus pauses dismissal. Click, Enter, or Space dismisses it. OS or in-game Reduced Motion removes travel/fade and uses immediate show/remove. Unlock, Palette Guide Off, screen exit, and unmount synchronously cancel it. This is not an `ENCOUNTERS`/`wj.tutorial` entry.
...
- **Persistent in-run table (changed 2026-07-28).** `ScreenTransition` is retired from blind↔shop↔pack↔blind-select changes and remains available only for shell navigation such as menu↔run.
  - **Shell transition surface (changed 2026-08-29).** The moving transition pane spans the full viewport; only its screen content remains constrained to the centered board width. Menu background light/grid effects also span the viewport, so neither resting screens nor transitions expose the board-width boundary as a vertical shading seam.
  - **Shell identity preservation (changed 2026-09-10):** captured outgoing and live incoming panes keep the same content-wrapper/provider hierarchy. The New Run selector therefore remains the same mounted instance through the slide and cannot flash its Continue-default state before leaving.
...
- Every game string through i18n (en/ko/ja/zh-CN/zh-TW/pt-BR/de/es-ES/fr-FR/ru-RU/pl-PL/tr-TR); add later languages through the shared registry and a matching locale file.
...
- **Score Keyboard jackpot/smoke follow-up (changed 2026-09-03):** red/yellow/green indicators use deterministic random-looking order plus rapid hard-edged jackpot pulses. Each overlay centre aligns with its painted indicator while the light casts wider horizontal pixel rays. Tier 3–5 pressed keys emit clearly visible smoke and Tier 4–5 pressed keys emit clearly visible flames with deterministic 0.95–1.05 per-key variation; Tier 5 applies ×1.35 smoke and ×1.25 flame base multipliers. Tier 4 adds seven distributed small chassis-smoke particles, while Tier 5 uses all twelve at a larger dark-gray size. No single smoke overlay is rendered.
