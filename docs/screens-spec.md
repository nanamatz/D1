...
- **Description face (changed 2026-07-31):** main and supplemental description plates use bundled Jost 700 with Noto Sans KR 700 fallback, compact tracking, and restrained cyan/magenta separation. This keeps Korean readable offline while giving effect copy the printed arcade tone of the reference.
  ...
  - **Text wrapping:** all main and supplemental tooltip copy wraps at word boundaries (`word-break: keep-all`), with emergency wrapping only for an unbroken token. Pouch selection and clicked-open pouch descriptions follow the same rule.
...
- **Run Info reference (changed 2026-08-04):** four tabs show Patterns, Word Hands, Blinds, and Vouchers. Word Hands lists all six hands in rank order with their condition, fixed Chips/Mult bonus, and whether gibberish qualifies. A hand with added Mult uses the one-line coloured `+Chips ×Mult` readout: Chips keeps its additive `+`, while Mult drops the `+` after `×`. When added Mult is 0, omit the `×0` axis and show only `+Chips`.
...
- **Open VFX (changed 2026-08-02):** reuse the selected pack illustration as two jaggedly clipped pieces. A short stepped rattle draws a bright horizontal tear, the illustrated top strip peels off, and seven navy pixel-card backs pour from behind the remaining wrapper amid flat gold/blue/red ink shards. The tear line fades out with the top/body pieces instead of lingering after the wrapper vanishes. The actual offered objects then fall into their final fan with a short stagger. Do not replace this with a generic scale-up, radial flash, smooth confetti, or unrelated box icon. Reduced motion skips directly to the final choices.
...
During word settlement, contribution beats run at **600ms per tile/effect at 1× speed** (rolled back from 800ms on 2026-07-29). Order is whole-word stamps → each played tile with its own enhancements and tile-triggered Emoji effects → global Emoji Tile list → held tiles frozen in visible play-time order → future consumable hooks → global/boss beats. The current tile lifts and bounces on every contribution. The settle-complete signal derives from this same cadence; 2×/4× game speed scale ordinary beats. A chance-bearing Lead Plate material beat keeps a **600ms real-time minimum** so its rolled value is visible even at 4×, and the same duration source delays completion accordingly. Enhanced tile triggers repeat their Chips/Mult/gold/retrigger value above the source tile.
...
|---|---|---|
| **Words** | profile challenge strip (highest-scoring word · longest word · most-played word · discoveries), then tabs: **Words** = tile-styled entries with search/suit filter and pagination; **Register Scores** = the live Standard/Formal/Slang/Vulgar multipliers and risk/reward copy | Both tabs share one fixed-height, internally scrollable content slot so switching never resizes the modal. Highest score uses only the intrinsic letter-chip sum; material/font/edition, Mult, Emoji Tile, boss, and sentence effects are excluded. Old settled-score records are recomputed from the word on read |
...
All 120 active Emoji Tile images use the shared `jokerArt` resolver on every rendered
surface.
