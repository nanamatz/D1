...
- **Game** (language moved to Main Menu 2026-09-08; speed scope changed 2026-09-06): game speed (ordered 0.5/1/2/4 — ordered score-event spacing only; 1 is default) · screenshake slider · reduced motion toggle (mirrors `prefers-reduced-motion`, user-overridable) · hint highlight color-blind-safe palette toggle · **"don't show tips" toggle** (kills the first-encounter tutorial popups and Palette reminders) · current-profile **Palette Convenience / 팔레트 편의성** action. The first action press only shows an inline `role=alert` irreversible warning; a second press of that same button grants exactly the four colors plus MUSIC/SOUND, never mascots or other progress. It persists the profile grant, durably absorbs the synthetic ids into any active/saved-run recap baseline, then emits one atomic aggregate event containing only actually added definitions. The global `ChromaticReveal` applies presentation and one fanfare once, reuses its 2.6-second/click-dismiss/Reduced-Motion grammar, and shows only `The world has been restored` / `세상을 되찾았습니다` / `世界を取り戻しました`; six individual word reveals are forbidden. Its `document.body` portal is layered above the in-run pause overlay and below tooltips so confirmation cannot expire unseen behind Settings. Tab change, Back, or unmount disarms it. Completed profiles show a disabled completed state.
...
All 14 adjustable row settings use the shared tooltip on row hover, native-control focus, and touch pin; Escape, outside tap, tab change, and unmount close it without consuming the setting action. The separate Palette action explains its irreversible effect through the two-press warning above.

**Palette reminder (width corrected 2026-09-09).** Once the guided YELLOW lesson is complete, the first normal settle of each blind may queue one non-blocking hint for a still-locked required Palette word, provided Tips is on and the board is input-stable. A required unlock on that submission or a blind-ending settle skips that blind. Visible copy uses one short localized `{word}` template, such as `“RED”로 해금`, instead of six per-word explanatory sentences. The six color/audio candidates circulate in registry order without repeating until every still-locked candidate has appeared; this memory is session-only and uses no game RNG. Pause, Run Info, Boss Intro, blind-entry effects, any coach-mark/chromatic reveal/modal, and non-playing phases suppress it. Render the complete notification as a keyboard-activatable button inside a polite status live region, portalled to `document.body` at the physical viewport's top-right. Its width shrink-wraps the localized text and close mark, with only a viewport-safe maximum that wraps unusually long translations. It slides in from the right for 240ms, holds for six seconds, then exits right for 180ms; hover/focus pauses dismissal. Click, Enter, or Space dismisses it. OS or in-game Reduced Motion removes travel/fade and uses immediate show/remove. Unlock, Tips-off, screen exit, and unmount synchronously cancel it. This is not an `ENCOUNTERS`/`wj.tutorial` entry.
