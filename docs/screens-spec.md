...
`wj.lang`, then a sanitized `en|ko|ja|zh-CN|zh-TW` hint only from a successfully initialized
...
breaks and omit terminal periods while preserving decimal points. Every numeric value is tagged for emphasis. English, Korean, Japanese, Simplified Chinese, and Traditional Chinese keys, interpolation
...
- Every game string through i18n (en/ko/ja/zh-CN/zh-TW); add later languages through the shared registry and a matching locale file.
