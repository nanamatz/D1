...
- **Score Keyboard / 점수 키보드 (progressive local tier + repeated clear celebration changed 2026-08-30):** the left margin owns one persistent non-interactive **1774×887, 2:1 landscape RGBA** vintage terminal keyboard with three empty raster wells. Raster + DOM keys rotate together 90° counter-clockwise into a vertical 1:2 dock: width `min(230px, 13vw, gutter − 16px, (100vh − 32px) / 2)`, height `width × 2`, centred, `contain`, never cropped, stretched, filtered, or made translucent. Its UI-only formula registry renders exactly 101 fully opaque labelled, disabled, untabbable keys: ESC/F1–F12, PRINT/SCROLL/BREAK, authentic no-Windows-key main block, ten nav/arrows, and 17-key numpad; A–Z appears exactly once and wide/tall keys retain ANSI-derived units. Every key is in the deterministic pool. Every `settleId` starts at Tier 0. Each score-increasing beat classifies `max(0, Chips × Mult + flat score) / positive finite target` for this submission only at above 1× and 1.25/2/4/10, then presents the maximum tier reached so far; decreases and zero/gold/stat/invalid beats cannot lower or falsely raise it, and committed/earlier/projection/sentence values are excluded. Eligible beats use that current peak with their semantic primary (letter/Stone→Space, global→Enter, untargeted Emoji Tile→stable F-key, boss→Break, Tag→Tab, Pouch→Space, fallback→Enter) plus deterministic fill to 3/5/8/12/16 visible keys and 1/2/3/4/5 sounds, primary first. Normal settle completion idles. An actual clear with a non-zero peak starts an immediate Enter-first synthetic cycle after settle completion and repeats during BUILD/LAND, final count-up, and verdict at the 460ms beat × Tier 1–5 factors `[2, 1.75, 1.5, 1.25, 1]`, divided by the clearing submission's snapshotted 1×/2× speed. Each cycle uses strong Enter + `deskEnter` first, deterministic key fill and `deskKeycap` for the remaining audible slots, local tier body/key shake, Tier-4 smoke, and Tier-5 smoke/flame/POP; tier light remains between beats. Fee Settlement, terminal modal, next blind, unmount, or Reduced Motion cancels all repeat timers/audio/VFX synchronously. Tier-0 clear, loss, sentence-only, and nonclear paths never loop. The exact target-cross event is still one logical once-per-blind Enter strike; celebrations intentionally reuse its sample. Reduced Motion preserves a static peak/Enter accent and the existing target-cross audio exception only. Committed count-up, Laboratory reuse, right-side encounters, `world-mono`, global shake, and settle timing/gate otherwise remain unchanged.
