...
`wj.lang`, then a sanitized `en|ko|ja|zh-CN|zh-TW|pt-BR|de|es-ES` hint only from a successfully initialized
...
breaks and omit terminal periods while preserving decimal points. Every numeric value is tagged for emphasis. English, Korean, Japanese, Simplified Chinese, Traditional Chinese, Brazilian Portuguese, German, and Spanish (Spain) keys, interpolation
...
- Every game string through i18n (en/ko/ja/zh-CN/zh-TW/pt-BR/de/es-ES); add later languages through the shared registry and a matching locale file.
