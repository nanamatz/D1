...
they never read profile unlocks or use `#unlock-chroma`. (changed 2026-08-30)
...
- **Final sentence-pattern callout and retained provenance (changed 2026-08-30):** when the last word's authoritative settle-complete signal lets BUILD publish a finalized real pattern, show one central localized `Lv.N · [pattern icon] pattern name` title using the existing 1.7-second Not Allowed animation family. Never show it for live projection, an ordinary word settle, or an Unison-only final. Reduced Motion uses a readable static/fade version and this callout never changes settle timing. The gold pattern stamp and separate Modifier, Mixed Register, Unison, effects, and Pouch tags match the Word-Hand activation tag footprint (`var(--fs-sm)`, `2px 9px`, 8px radius) and remain through BUILD, LAND, final count-up, verdict, and live Fee Settlement. Collect clears the snapshot in the existing Shop transition, and that null snapshot clears any still-running Reduced-Motion notice before Shop paints. Render order is Pattern → Modifier → Mixed Register → Unison → effects → Pouch. The reachable seven-tag maximum stays inside a bounded two-column region: provenance temporarily reallocates the fixed rail's score/controls heights from 136/332px to 216/252px without changing their 468px total; long Modifier, Mixed Register, flat-score effect, and Pouch tags span both columns for full English/Korean copy.
