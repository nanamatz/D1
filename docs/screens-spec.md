...
- **Guided intro copy (changed 2026-08-25):** the existing build step briefly says that during normal play, right-clicking a hand tile marks it for discard. This is copy only: the lesson remains three steps (frame → build → submit), keeps the YELLOW hard lock, and adds no discard action or selector.
