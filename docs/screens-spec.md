...
- **Guided intro action target (changed 2026-08-30; tracking corrected 2026-09-03):** the lesson remains four steps (frame → discard → build → submit), with the same real discard, exact physical-ID lock, resume/fallthrough, Skip cleanup, and settleId completion rules. Its three gated steps reuse one dynamic `.tutorial-action-target`: the unmarked spare is the sole right-click target; once marked, enabled Discard becomes the sole left-click target; build moves that left-click target through the exact Y→E→L→L→O→W IDs; submit moves it to enabled Play. `SpotlightBubble` retains its `document.body` portal, per-RAF target lookup/live viewport rect tracking, and 8px padding. The spotlight box has no positional transition, so it follows a newly assigned or animated target on the next paint without hover instead of chasing stale coordinates. Fine mouse pointers alone show a neutral, `aria-hidden`, pointer-inert 40×40 CSS mouse just outside and within 48px of the live target rect. Coarse pointers show none; Reduced Motion makes it static; its contrast remains readable in `world-mono`.
...
- **Will target-cross guard (changed 2026-09-02):** the once-per-blind target cue is enabled only by the authoritative post-boss `projectedScore ≥ target` state. Will remains one final Chips ×0.5 and Mult ×0.5 boss beat, and intermediate pre-halving axes cannot create a false cue; ordinary score typing is unchanged.
...
**Palette reminder (changed 2026-09-03).** Once the guided YELLOW lesson is complete, the first normal settle of each Chapter may queue one non-blocking hint for a still-locked required Palette word, provided Tips is on and the board is input-stable. A required unlock on that submission or a blind-ending settle skips the Chapter. The six color/audio candidates circulate in registry order without repeating until every still-locked candidate has appeared; this memory is session-only and uses no game RNG. Pause, Run Info, Boss Intro, blind-entry effects, any coach-mark/chromatic reveal/modal, and non-playing phases suppress it. Render the complete notification as a keyboard-activatable button inside a polite status live region, portalled to `document.body` at the physical viewport's top-right. It slides in from the right for 240ms, holds for six seconds, then exits right for 180ms; hover/focus pauses dismissal. Click, Enter, or Space dismisses it. OS or in-game Reduced Motion removes travel/fade and uses immediate show/remove. Unlock, Tips-off, screen exit, and unmount synchronously cancel it. This is not an `ENCOUNTERS`/`wj.tutorial` entry.
