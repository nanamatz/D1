...
fixed tab row, internally scrolling content viewport, `82px` primary-action row
with a centred action capped at `340px`, always-reserved optional-note row, and
one full-width Back row. Tab changes and conditional seed/Challenge notes never
...
- Seed controls remain inside the New content viewport. Their conditional warning
  uses the reserved note row; the centred blue **Play** and full-width gold
  **Back** remain in the shared fixed rows.
