...
| **Enhanced Tiles** | two paged views using the shared `< · page · >` footer: page 1 Materials = 9 tile faces (base + 8 enhanced, including Wood); page 2 Fonts = 5 Futura variants | both pages render large pixel-art tile swatches; Wood tooltips show `Currently +N Chips / 현재 +N 칩` from the live instance (generic Collection/reference copy uses base +15) plus +10 per play, without “during this run / 이번 런 동안”; Font tooltips show each seal effect from `balance.ts` `fontEffects` (changed 2026-08-31) |
