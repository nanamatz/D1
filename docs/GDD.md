...
- **Target-cross cue.** The Score Keyboard strikes its main Enter key once per blind and plays the distinct `deskEnter` bottom-out/return on the exact live word/sentence count-up crossing. This logical cue is presentation-only and independent of the submission-local peak. After settle completion, a qualifying clear celebration may intentionally reuse the same `deskEnter` sample on each repeated Enter-first cycle; that does not retrigger the once-per-blind crossing event. Resolution still waits for the authoritative settle-complete signal plus the verdict beat.
