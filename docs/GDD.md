...
> **Displayed round score = committed ONLY (playtest-04 A; transfer changed 2026-09-04).** The big round number on screen is the **committed** score (layer 1) and **never decreases**. During a full-motion word settle it holds at the pre-word committed value; when the authoritative settle-complete signal fires, the committed word gain appears without a plus sign in the always-reserved 52px status slot above Chips × Mult, counts down to zero, and transfers into the round number as that number counts up over the existing 700ms tween. The transfer derives only from `max(0, committedScore − committedBefore)`, including any flat score already committed; it never reconstructs Chips or Mult, adds a timer/event, or changes `settleDurationMs()`. Invalid, zero, and negative gains display nothing. Reduced Motion instead holds the full gain statically during the existing 700ms reduced settle and then snaps the round number to committed while removing the gain. A clearing last word completes this transfer during sentence BUILD before the existing sentence LAND. The empty 52px slot remains through BUILD so the ScoreBox does not move when the transfer finishes. Beside the round number, the UI shows the **current highest valid sentence pattern** and its live sentence-bonus score as `pattern name : score` (for example, `의문문 : 120`). That score is still a projection which may be overwritten as the sentence changes; it is never folded into the round number during play. The engine uses the same projected score for auto-clear, and the bonus resolves visibly in the settle sequence (§7.2) when it is the deciding factor.
...
**Each phase:** submit word → settle & accumulate individual score (letter × suit multiplier × emoji tiles) while the displayed round holds at its pre-word committed value → re-judge sentence with current sequence → display the current highest valid pattern name while updating projected score internally → once the full settle sequence has played, transfer the authoritative committed gain from the always-reserved 52px status slot into the round total over the existing 700ms count-up → if projected ≥ target, detect the clear and complete that word transfer during BUILD before the sentence bonus LAND; after LAND and a short beat, auto-resolve to Fee Settlement (§7.2 — no early-end button, no intermediate verdict screen).
...
**Endless curve (implemented 2026-07-31; number presentation changed 2026-09-04).** For Chapter `a ≥ 9`, let `c = a − 8`. The base target is `105000 × (1.6 + (0.75c)^(1+0.2c))^c`, truncated downward to two significant digits before blind, Pouch, Record, and boss multipliers apply. This double-exponential curve deliberately outruns slot-limited scaling. Chapter 38 is the explicit finite-number endpoint: clearing its Deadline ends the endless run; Chapter 39 is never created. Shared score/money text rounds once to integers, comma-formats values below 1,000, and otherwise uses the largest `K/M/B/T/Qa/Qi` unit with at most one decimal truncated toward zero; values from `1e21` retain compact scientific notation, and non-finite values show an em dash. The main ScoreBox keeps Chips and Mult as their authoritative independent axes and sizes each formatted visible string without recomputing the values: up to 7 characters uses 52px, 8–9 uses 38px, and 10+ uses 30px. Profile statistics track the best finalized blind score across every mode; Lifetime statistics separately track the highest Endless Chapter and best finalized Endless blind score.
