...
> **Score Typewriter (unlabeled authored caps revised 2026-08-29).** A visually quiet pixel-art typewriter stays idle in the viewport's left margin throughout the in-run flow and reacts only while score resolution is playing; it is an object-only image, never a desk/background vignette, and runtime random desk encounters use the right margin only. Its locally bundled **320×1760 transparent RGBA master is a body-only chassis with no paper and no baked keycaps**, showing the deliberately elongated mechanism rotated 90° counter-clockwise. The hard-edged, limited palette matches the game screen and avoids bright continuous metal highlights. The dock always occupies 94% of the viewport height and the safe available lane width; the 2:11 master is intentionally scaled to that responsive rectangle with `object-fit: fill`, prioritizing whole-panel coverage over mechanically exact proportions while never crossing the central-board boundary. It remains below transitions, modals, tutorial and tooltips, and the filtered raster stays lower-contrast than the central score HUD until its score overlays fire. Its invisible positioning plane holds **27 unlabeled, disabled, untabbable native DOM buttons**, each with an individually authored x/y position, scale, and tilt. The loose 10/9/8 curved mechanical banks use no row/group wrapper and render no key legend. Every cap is navy/charcoal with a muted metal rim, one-pixel highlight, and hard bottom/right shadow; permanent gold or ordinary UI-button styling is forbidden. Each presentation beat independently measures `delta = abs((chipsAfter × multAfter + flatScoreAfter) − (chipsBefore × multBefore + flatScoreBefore))` from the unrounded local axes in their existing presentation order. Its frozen comparison base is `max(60, letterChips(played tiles) × (base suit Mult + wordLengthMult(scoringLength, isGibberish)))`; sentence finalization reuses the last submission's frozen base and snapshotted settle speed through BUILD/LAND. Impact tiers are Idle for non-finite/zero delta, then Typing `<0.25`, Fast Typing `<1`, Frenzy `<3`, Overdrive `<10`, and Meltdown `≥10`. Negative effects use absolute magnitude while their existing source readout keeps the sign. Round total, committed/projected score, target, and overshoot never enter this classification. A deterministic 32-bit beat hash produces the unique cap permutation and uneven rhythm; the configured `0.35` jitter is normalized so every final release remains inside the snapshotted 1×/2×/4× beat. The five active tiers press 3/5/8/12/16 visible caps while reserving the existing 1/2/3/4/5 audible density at matching visual press times. Only the selected cap itself travels inward 2–3px, scales slightly, collapses its shadow, and gains a subtle dull-gold face. Crossing the target instead fires one separate typewriter bell + punch and scoring continues; it neither raises the tier nor bypasses the settle-complete signal. Reduced Motion cancels key SFX and all key/carriage/body/shake/smoke/flame/POP motion; only the selected caps retain a low-intensity static metal accent, with no rectangular well cue. These effects add no timer to `settleDurationMs()` and never step the committed-only round number.
> The DEV-only Laboratory mounts that same component in its reserved lane for local Idle/Tier/target/speed/shake/Reduced Motion previews; it never constructs engine state or writes settings, run, RNG, storage, or profile data. The separate DEV unlock-recap preview is retired, while the integrated run-end recap remains the real player-facing path. (changed 2026-08-29)
...
- **Target-cross cue.** The Score Typewriter rings and punches once on the exact live word/sentence count-up crossing. This cue is presentation-only: remaining score beats continue and resolution still waits for the authoritative settle-complete signal plus the verdict beat.
...
The playable world begins **desaturated and its gameplay mixer silent**; playing specific words permanently unlocks presentation layers. The pre-game Sweet Turtles developer ident is the sole audio exception and is fully separate from that mixer (C-6). Its physical-viewport backdrop is literal `#000` and stacks above the global CRT layer, so it is also deliberately independent of the progressive palette and board-background tokens. This is the literal enactment of the title — you *play the world into existence*. Persistent **per profile** (localStorage `wj.unlocks`, beside collection/tutorial flags). **Valid words only** unlock (gibberish never does).
...
| YELLOW | gold token group — money, gold UI, target-cross/typewriter accents |
...
**Audio gating (C-6, changed 2026-08-29).** MUSIC/SOUND gate the shipped gameplay mixer's buses — **default off** (play begins silent) until the word is played or the override is on. Color groups are independent. The automatic pre-game Sweet Turtles developer ident is the sole exception: its original synthesized cue owns a separate Web Audio context and fixed peak-safe output, and never imports, reads, or obeys the gameplay mixer, settings/storage, Master/SFX/music levels, mute, or any Palette audio gate. It attempts ungestured playback once without delaying the visual; blocked web autoplay fails closed with no later-gesture queue, while the Electron shell enables Chromium autoplay before app readiness/window creation. Image failure or preparation timeout uses the silent text-only fallback.
...
**Discoverability (C-5).** New Collection category **팔레트 (Palette)** — locked entries are grey silhouettes with a letter-count hint ("R _ _"), unlocked entries show the word in its group color. The first-run tutorial is a scripted, **hard-locked YELLOW lesson**: the opening hand is rigged to contain Y‑E‑L‑L‑O‑W. A WooDak coach-mark frames the grey world, then teaches the ordinary two-action discard before word-building: the first six physical YELLOW IDs are protected, the next opening-order spare is the sole right-click mark target, and the existing red Discard button performs the real budget-spending, seeded refill. Only successful discard state advances; a completed prior discard or impossible discard safely resumes at build. The player then builds those exact six IDs and submits YELLOW; the yellow palette washes in. All unrelated input stays locked, and Skip releases the lock without leaving an uncommitted mark. The target is **not** lowered — it remains 300, so YELLOW (252 under §3.1) ends the lesson but does **not** clear the blind. This remains UI policy over generic `openingLetters` and ordinary `discardTiles`; it adds no engine tutorial state. `tests/yellow-lesson.test.ts` guards the physical-ID, real-discard, determinism, and target invariants. (discard step added 2026-08-27.) See `docs/superpowers/specs/2026-07-21-yellow-first-lesson-design.md`.
