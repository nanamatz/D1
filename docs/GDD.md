...
**Score Keyboard panel LEDs (changed 2026-09-03).** Tier 5–6 active and clear-hold states blink the three red/yellow/green indicator LEDs painted in the keyboard image, with a deterministic random-looking staggered order that changes each beat/cycle. Tiers 0–4 leave them off. The former full-keyboard rainbow ring and Tier 6 jackpot sparks are retired. Panel LED and per-key flame colours remain fully coloured under Palette progress and `world-mono`; Reduced Motion freezes the panel LEDs at low intensity and Forced Colors hides them.
...
> **Rainbow key lighting override (changed 2026-09-02; panel order changed 2026-09-03).** The 101 physical caps have immutable six-slot rainbow assignments. Every actually pressed key emits its own hard-edged pixel LED into the surrounding gaps; idle and unpressed keys emit none. Tiers 5–6 deterministically include all six colours and add one timing-synchronised flame on every pressed key, replacing the former single central flame. Tier 6 flames are 25% larger and may throw one ember pixel; Tier 4 keeps machine smoke, and Tiers 5–6 keep smoke and POP. The keyboard image's red/yellow/green panel indicators also blink on Tier 5–6; their three staggered phases use a deterministic random-looking order per beat/cycle, while Tiers 0–4 stay dark. The former full-keyboard rainbow ring and Tier 6 jackpot sparks are retired. These LED/flame colours are an explicit exception to §13: they remain fully coloured under `world-mono` and at every chromatic-unlock state. Reduced Motion removes moving LED/flame VFX while retaining low-intensity panel LEDs plus the existing static pressed/Enter accent, and Forced Colors hides the decorative LED/flame. No key count, audible count, sampled-key mapping, repeat lifecycle, audio, or settle timing changes.
...
Korean, `japanese` maps to Japanese, `english` maps to English, and every unsupported
or failed lookup falls back to English. The renderer receives only `en|ko|ja`, never the raw Steam locale or
...
the saved choice or English. (Japanese localization added 2026-09-06.)

> **Score Keyboard jackpot/smoke follow-up (changed 2026-09-03).** Red/yellow/green panel LEDs use the existing deterministic random-looking order with rapid hard-edged jackpot pulses. Each overlay centre aligns with its painted indicator while the light projects wider horizontal rays. Tier 4–6 smoke rises above each pressed key; key smoke and flame retain deterministic 0.95–1.05 variation on larger visible bases, while Tier 6 applies ×1.35 smoke and ×1.25 flame base multipliers. Tier 5 adds seven small smoke sources distributed over the keyboard chassis; Tier 6 uses all twelve, larger and dark gray. This is particle CSS, never one large smoke image.
