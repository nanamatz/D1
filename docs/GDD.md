...
**UI text-selection suppression (changed 2026-09-09).** Pointer dragging across the game never produces browser text-selection highlighting, including in body-portalled presentation layers. Editable text controls retain native selection.

**Back action icon (changed 2026-09-09).** Localized Back labels contain no arrow characters; every Back button prepends the same decorative pixel left-arrow icon, leaving the spoken accessible name as the localized action label.

**Score Keyboard panel LEDs (changed 2026-09-03).** Tier 4–5 active and clear-hold states blink the three red/yellow/green indicator LEDs painted in the keyboard image, with a deterministic random-looking staggered order that changes each beat/cycle. Tiers 0–3 leave them off. The former full-keyboard rainbow ring and jackpot sparks are retired. Panel LED and per-key flame colours remain fully coloured under Palette progress and `world-mono`; Reduced Motion freezes the panel LEDs at low intensity and Forced Colors hides them.
...
- Changed 2026-09-10: gibberish holes now split the blind history into independent sentence candidates. Each non-empty candidate is scored from only its own word subtotal; the greatest actual sentence gain wins, with ties resolved by pattern rank, then candidate length, then earliest position. Only that one gain finalizes (§§5.1–5.3, 6.4, 7.1–7.4).
...
- New: **Blinds, Antes & Bosses** — scaling, Chapter-8 victory + Endless Mode, 18 regular bosses, and a seven-boss finisher tier every eight Chapters (§8.4). Draft/Revision skipping and 30 Editorial Perks now ship (§8.2; boss pools changed 2026-09-09).
- Changed 2026-09-09: Family Photo, No Smoking Sign, Wi-Fi Zone, and Transparent Paper extend the ordinary pool with adjective/adverb, noun, preposition/conjunction, and interjection debuffs (§8.3).
- New: **Shop & Economy** — money sources, interest, shop layout, packs, 32 two-tier vouchers.
- Changed 2026-09-06: Uncommon and Rare Tags replace ordinary item-slot offers instead of adding extra shop products, so tagged Emoji Tiles always remain within the current item-slot count (§8.2, §9.2).
...
| money (`gold`) | **돈** | **Fee** | `$` symbol stays |
| Cash Out screen | **돈 정산** | **Fee Settlement** | |
...
Effects are **per tile** and stack: three Porcelain tiles in one word give +90 Chips; two Ivory tiles held at blind end pay $6. Wood growth is stored on that individual tile and survives blind transitions for the rest of the run. The headless submission result advances a played Wood tile exactly once and publishes that same grown snapshot to the permanent pouch, hand/bag/discard state, sentence sequence, submitted mini, and settlement UI. A concrete Wood tile tooltip shows one combined current Chips total (letter Chips + live Wood bonus), while generic Wood references state only the +10-per-play rule. (changed 2026-09-04)
...
- **Use the playable ENABLE subset plus exact curated registries (changed 2026-09-08).** The validity pool contains every ENABLE word of **18 letters or fewer**, 13 apostrophe-free tile-grammar exceptions, exactly four curated acronym surfaces (`mvp`, `mvps`, `vip`, `vips`), and exactly four reviewed omissions (`aint`, `christ`, `christmas`, `june`): **172,236 dictionary words and 172,259 baked lexicon entries**, including 23 retained entries outside the dictionary. The four acronym rows plus `christ`, `christmas`, and the month noun `june` are Standard nouns; `aint` is the apostrophe-free AIN'T surface, Slang with the existing auxiliary/linking-verb POS convention. `lexicon-pipeline/curated-abbreviations.json` remains the exact-four acronym source; `lexicon-pipeline/curated-validity.json` is the separate reason-bearing source for reviewed ordinary omissions. Both baked outputs consume both registries deterministically. The latter admits only its exact lowercase spellings: it does not make proper names generally valid and does not generate plurals, derivatives, or related forms. Punctuation and unlisted initialisms such as CEO remain invalid. The 608 longer ENABLE words are excluded because 18 is the designed playable-word ceiling after Copy Editor is considered. The former 50k frequency cutoff remains retired; it excluded valid base words such as `uremia` while retaining derivatives such as `uremic`.
...
All ten POS ids have distinct presentation tokens and final colours, including the three verb subtypes and the former adjective/adverb/article and conjunction/preposition groupings. Palette ownership remains BLUE for noun/interjection, RED for verbs, YELLOW for adjective/adverb/article, and GREEN for conjunction/preposition; their neutral locked values also remain distinguishable by luminance. Labels plus solid active/dashed alternative borders remain authoritative in monochrome and forced-colours modes. (changed 2026-08-30)
...
1. **Hole-delimited whole-sentence match.** Apply any boss sentence transform to the raw submission history first (so Orphan Line always removes the literal first submitted word), then remove every debuffed submission and join the remaining eligible words in their original order. A debuffed word is not a hole and contributes neither POS nor register. Each gibberish hole (§6.4) splits that history; every non-empty segment on either side becomes an independent sentence candidate whose entire sequence must match. No partial matching within a candidate. (changed 2026-09-10)
2. **One winning sentence and pattern only.** Within each candidate, only its highest-ranked matching pattern applies. Score each candidate independently from its own settled word subtotal, including pattern level, modifiers, Unison/register synergy, and sentence-scoring hooks. The greatest actual sentence gain wins; ties resolve by higher pattern rank, then more words, then the earlier candidate. Holes and losing candidates keep their individual word scores only. Candidate gains are projections and never accumulate. An all-gibberish or boss-removed history has no candidate and therefore no sentence bonus; a truly empty blind keeps the legacy no-pattern settlement path. (changed 2026-09-10)
...
4. **POS compatibility follows the winner.** POS highlighting is derived from the winning candidate's highest-pattern judgment, never reconstructed in the UI. For each word in that candidate, every lexical POS candidate that preserves the exact winning pattern outcome (including its modifier/repeat result) remains active; this keeps the union of equivalent parses instead of choosing an arbitrary verb subtype. Words outside the winning candidate, plus boss-transformed or debuffed-out raw words, have no selected POS. This compatibility trace is ephemeral and does not change scoring, rank, or stored submissions. (changed 2026-09-10)

### 5.2 The Twelve Patterns (weak → strong)

Every pattern owns a base **[Chips × Mult]** pair (Balatro-hand style). Each hole-delimited candidate uses only the settled word subtotal inside that candidate as its Chips axis: pattern, modifier, and Unison Chips add first; an active mixed-register synergy multiplies that combined Chips axis and is materialized as sentence Chips; post-pattern hooks then add Chips or multiply Mult. The candidate with the greatest resulting gain supplies the blind's sole sentence bonus; scores outside it are never multiplied. This makes structural scoring scale with the words that formed the sentence instead of unrelated earlier or later submissions. Base Mult uses the weakest Outcry as its ×2 floor, then stays within narrow ×2/×3/×4 rank bands so the strongest base pattern's gain is approximately three times the weakest rather than orders of magnitude larger. The first level-up adds the pattern's construction-difficulty tier Chips increment (**Easy +15 · Medium +30 · Hard +45**) and +1 Mult; each later level-up grows both increments by ×1.5 (`BALANCE.patternLevelGrowthFactor = 1.5`). Current totals and displayed level-up deltas are rounded to natural numbers. Rank remains payout precedence, not a proxy for construction difficulty. (changed 2026-09-10)
...
segmentWordScore    = sum of settled word scores inside this candidate only
scaledSentenceChips = (segmentWordScore + rawSentenceChips) × registerChipsFactor − segmentWordScore
sentenceChips       = scaledSentenceChips + post-pattern Chips
sentenceMult        = patternMult × unisonMult × post-pattern Mult
candidate total     = (segmentWordScore + sentenceChips) × sentenceMult + post-pattern score
candidate gain      = candidate total − segmentWordScore
final blind score   = committedScore + greatest single candidate gain
```
...
Stage preview, failed input, a debuffed/removed word, a losing sentence
candidate, and a lower-ranked overlapping parse do not reveal it. Once revealed, the row remains for the
...
**Run Info visual examples (added 2026-09-03).** Each visible Patterns-row tooltip
keeps its rule description and adds the official §5.2 example as an ordered tray
of uppercase word boxes only, with no Example heading, divider, role/POS labels,
or clause captions. Each ordinary box uses its selected POS tag colour for its
fill and border; a special grammar token such as ISNT uses the neutral marker tone
instead of an invented POS colour. Every example, including Compound and Complex,
lists all word boxes left-to-right in one uninterrupted row with no arrows or row
breaks. The row fills the tooltip width, centres the group, and distributes the
boxes evenly. One localized accessible summary reads the example words. The tray is
UI-only and is constructed after the discovery filter, so a hidden pattern
contributes no example markup or accessible text until revealed. (simplified
2026-09-04)
...
- **The Chips×Mult ladder climbs together** — both sides grow from #1→#12, so structural sentences add more Chips and apply a stronger factor to their own segment subtotal. The "structural sentences pay off big" principle from §7.3 lives directly in both axes.
...
> **Unison.** If a candidate has 2+ words and *all* its words share one suit, a bonus applies, sized by suit rarity: **Standard +50 Chips · Formal ×1.25 · Slang ×1.5 · Vulgar ×2** (placeholders).

Unison folds directly into the §5.2 formula: **Standard adds to the Chips side; Formal/Slang/Vulgar multiply the Mult side** (values unchanged). A register-mult Unison therefore scales that candidate's word subtotal even when no sentence pattern matches. This preserves the flush role ("commit to one suit across phases → reward"), while the mixed set below rewards deliberate contrast.

If no Unison exists, the unordered union of a candidate's final effective-register memberships may trigger exactly one mixed synergy. Order, word count beyond the two-word minimum, and repeated registers do not matter; the exact final set does. The remaining two-register pairs—Standard+Slang, Standard+Vulgar, and Formal+Slang—grant nothing.
...
The factor multiplies `segmentWordScore + rawSentenceChips` before sentence-scoring hooks. Its gain is materialized into `sentenceChips`, so existing Emoji Tile, boss, Briefcase, projection, and final-settlement paths stay authoritative. A pattern may coexist with one mixed synergy. Unison and mixed synergy are mutually exclusive, with Unison priority whenever all candidate words share any final register. Thus Tower of Babel remains Unison-only. A gibberish hole ends both judgments for that candidate and starts a fresh candidate after itself. (changed 2026-09-10)
...
- **Shipped-lexicon check (2026-09-08).** Among 172,259 valid entries, Vowelless has 60 candidates under the live Y-as-consonant rule (the hypothetical Y-as-vowel branch has 17), Type Economy has 10,164, and Grand Palindrome has 7. The four curated acronym surfaces plus `aint`, `christ`, `christmas`, and `june` qualify for none of these hands, so the seeded 100,000-hand upper-bound rates remain roughly 1.56%, 7.56%, and 0.017% respectively; these are perfect-dictionary-solver rates, not expected human completion rates.
...
- **Sequence effect (b-2):** the gibberish entry is recorded as a **hole** in the sentence history. It ends the current sentence candidate and lets the next valid submission begin a fresh one. Candidates on both sides remain eligible, but only the greatest actual sentence gain finalizes (§5.1).
...
- **UI note:** the tray keeps the hole visually distinct without a secondary boundary label. The live pattern label keeps the best candidate so far and changes only when a later candidate overtakes it.
...
- **Layer 2 — sentence bonus (projected → final).** Each phase, the hole-delimited sentence candidates so far are judged (§5) and the greatest actual candidate gain becomes the projection — **overwrite, not accumulate**. Only that one gain is finalized when the blind ends.

> **Owned Emoji Tile order (changed 2026-09-03).** In the global owned-list `wordScoring` stage, resolve physical owners from left to right. Within each owner, apply and present its Gray/Violet/Rainbow scoring edition first, then its intrinsic effect, then any growth/decay beat before advancing to the next owner. Base and White add no scoring-edition beat. Tile-bound Emoji effects remain attached to their earlier played-letter-tile phase and are not moved by this rule.

> **Displayed round score = committed ONLY (final-settlement axes clarified 2026-09-06).** The big round number on screen is the **committed** score (layer 1) and **never decreases**. During a word settle it holds at the pre-word committed value while the ScoreBox axes replay the ordinary word's Chips and Mult beats. When no finalized sentence bonus or settle is active, staging tiles that form a Word Hand displays that preview's exact bonus Chips and Mult on the ScoreBox, including ×1; a debuffed or handless preview displays 0 × 0. Hidden profile-secret Hand names remain `???`, but their earned numeric preview is not hidden. When the authoritative settle-complete signal fires, a positive finite committed word gain appears without a plus sign in the always-reserved 52px status slot while Chips × Mult already reads 0 × 0. The gain and round hold unchanged for a fixed 500ms at every game speed; then the gain counts down to zero as the round counts up over the existing 700ms tween. The transfer derives only from `max(0, committedScore − committedBefore)`, including any flat score already committed, and never reconstructs Chips or Mult. Invalid, zero, negative, and non-finite gains display nothing and add no hold. Reduced Motion keeps the same static 500ms post-settle hold at 0 × 0, then removes the gain and snaps the round to committed without a tween. On a full-motion clearing word with a real sentence bonus, the word transfer runs alone from t500–1200 with `sentenceBonus` still absent and Chips × Mult at 0 × 0; BUILD publishes at t1200 while the axes remain 0 × 0, then LAND runs through t2600 before the verdict beat. Final sentence settlement never replays the committed round score as Chips or Mult. A positive no-bonus clear may publish its final score at t500 because the existing resolution delay still protects the transfer. Zero or non-finite gain retains immediate sentence processing. The UI-only timers are keyed by `settleId` and cancelled on mode change or unmount. They add no score event and change no settle-complete signal, `settleDurationMs()`, input, audio, shake, RNG, engine state, or save state. Every status variant is vertically centred in the fixed 52px slot; the enlarged Word-Hand label keeps a safe line box, top clearance, and one-line ellipsis under Forced Colors without moving the ScoreBox. The empty slot remains through BUILD so the ScoreBox does not move when the transfer finishes. Beside the round number, the UI shows the **current highest valid sentence pattern** and its live sentence-bonus score as `pattern name : score` (for example, `의문문 : 120`). That score is still a projection which may be overwritten as the sentence changes; it is never folded into the round number during play. The engine uses the same projected score for auto-clear, and the bonus resolves visibly in the settle sequence (§7.2) when it is the deciding factor.

> **Candidate winner clarification (changed 2026-09-10).** “Current highest valid sentence pattern” above means the pattern belonging to the candidate with the greatest actual sentence gain, not simply the highest pattern rank anywhere in history. A later candidate replaces the displayed winner only when its gain is greater, with pattern rank, length, and earliest-position tie-breaks.

> **Final pattern callout and provenance lifetime (score-panel layout changed 2026-09-04).** Only after the authoritative last-word settle-complete signal lets BUILD publish the finalized `SentenceBonusDisplay`, a real pattern receives one central localized `Lv.N · [pattern icon] pattern name` title callout using the 1.7-second Not Allowed motion family. This is not the live projection and adds no engine event, score, RNG, or settle delay. The reserved live-pattern line becomes the finalized Pattern/Unison/Mixed headline described in §5.2, with no duplicate score-box stamp. The score panel keeps its 52px status/transfer slot present and empty during BUILD so the ScoreBox never moves. Modifier, grouped Effects, and Pouch render as zero to three full-width supplemental rows, separated visibly from the ScoreBox. Zero or one row keeps the score/controls panels at 188/296px; two rows use 211/273px; three use 234/250px, preserving their 484px total without blank space. The snapshot remains visible through BUILD, LAND, final count-up, verdict, and live Fee Settlement. Collect clears it in the existing same-transition Shop reset; it is not added to persistence. Reduced Motion keeps a readable static/fade form.

> **Presentation speed baseline (scope changed 2026-09-07).** Public game speed is exactly 0.5×, 1×, 2×, or 4× in that order, with 1× as the default, and affects only spacing between ordered score-calculation events. Ordinary slots are `600ms / speed`; enhanced owned-Emoji-edition slots are `1000ms / speed`; Lead Plate chance slots keep a 600ms real-time floor and creation score slots keep a 480ms floor. Every submitted row uses the same fixed 650ms anticipation/drop/contact/recoil impact at every speed, with the shared contact tick at 40%; every material-family burst starts there, with at most one settings-scaled screen shake and one `submitThock`. The final tally hold is fixed at 650ms when score beats exist, while a settle-only/debuffed play remains impact-only. Tile/source VFX, audio, shake lifetime/intensity, the Score Keyboard's 1.5×-faster Tier 1–5 key/clear-repeat timing and fixed target cue, the 500/700ms score transfer, BUILD/LAND, notices, verdicts, modals, transitions, and pack opening all remain independent of game speed. Feedback still starts on its source score event; there is no separate queue. Reduced motion stays fixed at 700ms with only a static contact highlight. `settleDurationMs()` remains the single settle-complete source: fixed impact + scaled event slots + fixed final hold. Speed is snapshotted per submission only for event cadence; Reduced Motion ON switches an active submission to the fixed branch, and OFF applies from the next submission. Exact supported stored values remain unchanged; strings, non-finite values, and unsupported numbers normalize to 1, while a persisted 2 remains 2. (The group treatment still supersedes the 2026-08-22 per-tile stagger.)

> **Score Keyboard / 점수 키보드 (typing speed changed 2026-09-08).** A solid fictional vintage terminal keyboard stays idle in the viewport's left margin and reacts only during score resolution; runtime random desk encounters remain right-only. Its locally bundled **1774×887 transparent RGBA master** is a natural 2:1 landscape object with three flat key wells and no baked caps or legends. The complete raster + DOM-key wrapper rotates 90° counter-clockwise into a vertical 1:2 dock: width is `min(230px, 13vw, gutter − 16px, (100vh − 32px) / 2)`, height is `width × 2`, it is vertically centred, and `object-fit: contain` may never crop, stretch, filter, or change object opacity. One UI-only formula/row registry supplies **101 fully opaque, labelled, disabled, untabbable ANSI-derived caps**: ESC, F1–F12, PRINT/SCROLL/BREAK; the standard main block without Windows keys; ten navigation/arrow keys; and a 17-key numpad. A–Z appears exactly once. Authentic proportions include 2u Backspace, 1.5u Tab, 1.75u Caps, 2.25u Enter, 2.25u/2.75u shifts, 7u Space, 2u-wide numpad 0, and 2u-tall numpad +/Enter. Every cap belongs to the deterministic candidate pool. Each `settleId` starts at Tier 0. A positive current-submission score beat activates classification. Its numerator is `max(0, Chips × Mult + flat score) + sentenceAssist`, where `sentenceAssist = max(0, projectedScore − committedScore)` is captured from the visible post-submit blind state once per `settleId` and is zero under `previewHidden`. Divide by the positive finite `blind.target`: Idle is `r < 1`; Tier 1 is `1 ≤ r < 1.2`, Tier 2 `1.2 ≤ r < 1.3`, Tier 3 `1.3 ≤ r < 1.4`, Tier 4 `1.4 ≤ r < 1.5`, and Tier 5 `r ≥ 1.5`; present `max(previous peak, raw)`. A decrease, zero-score, gold-only/stat-only beat, invalid value, or sentence assist alone cannot lower or falsely raise that peak. `committedBefore` and earlier submissions never enter directly, and later sentence BUILD/LAND cannot reclassify it. Every eligible beat uses its current peak and includes one semantic primary before deterministic fill: tile/tile-bound beats use their submitted A–Z key, letterless Stone uses Space, suit/word-length/Word-Hand/global use main Enter, untargeted Emoji Tiles use stable F1–F12, boss uses Break, Tag uses Tab, Pouch uses Space, and unknown uses Enter. Tiers use 5/10/12/16/20 visible caps and 3/4/5/6/7 audible presses, with the primary audible first; every Tier 1–5 key sequence and press runs at a 1.5×-faster fixed `418ms / 1.5` (≈279ms) beat independent of selected speed. The audible slots resolve the same visual key sequence and use a locally bundled 32-WAV pool: registry index i maps to sample (i mod 32) + 1, and an unknown id falls back to main Enter. Samples load only when played; a failed sample uses the matching existing synthesized recipe. Normal submissions return to idle on settle completion. For an actual clear with a non-zero peak, including one raised by sentence assist after a positive local beat, settle completion immediately starts synthetic `clear:{blindKey}:${settleId}:${cycle}` score beats through sentence BUILD/LAND, final count-up, verdict, and the live Fee Settlement row/total animation. Collect's transition to Shop, a terminal modal, or the next blind cancels them in the same render; unmount and Reduced Motion ON also cancel synchronously. This extension is Score Keyboard-only: temporary settle/tile/Emoji VFX still finish on their ordinary timelines. Main Enter is always the primary and first audible slot, using Enter's mapped WAV at the existing `deskEnter` accent gain; deterministic key fill uses each visible key's mapped WAV at the ordinary `deskKeycap` gain to complete the tier's 3/4/5/6/7 audible count. The repeat interval is the 1.5×-faster fixed `418ms / 1.5` (≈279ms) score-keyboard beat times `[2, 1.5, 1.25, 1, 1]` for Tiers 1–5, independent of selected speed. Every cycle replays every selected key's tier-scaled local body/key motion with a clearly visible 4px/.86 physical key depression; Tier 3 adds smoke and Tiers 4–5 add smoke, flame, and POP; Held tiers retain their 1.5×-faster LED/smoke cadence, while the former gold chassis border and glow are removed. Tier-0 clear, loss, a true local-zero sentence-only clear, and non-clearing submissions never loop. Crossing the cumulative live target is still one logical once-per-blind main-Enter event, but its visual/audio cue is armed only when the engine's post-boss `projectedScore` authoritatively clears the target; intermediate axes before Will's final Chips ×0.5 and Mult ×0.5 beat never create a false crossing. Ordinary submission typing is unchanged. Enter's mapped WAV at the `deskEnter` accent gain is intentionally reused by each later clear-celebration cycle. Reduced Motion before or during the hold synchronously cancels repeat timers, audio, and moving VFX, retains only the static peak/Enter accent, and preserves the existing one-shot target-cross audio exception; turning it off applies from the next submission. These effects add no timer to `settleDurationMs()`, never trigger global screen shake, never step the committed-only round number, and never bypass settle-complete.
> **Rotating developer Laboratory (changed 2026-09-07).** The DEV-only Main Menu Laboratory is not a permanent feature gallery. Whenever a completed player-facing implementation or change cannot be inspected immediately in Collection or Run Info, its current contents are replaced by that one review surface; work already reachable there and internal-only refactors do not enter the Laboratory, and automatic tests retain coverage for retired previews. It reuses production components and data rather than copying them, and never constructs or mutates engine/RNG/run/discovery/profile/settings/storage state. Its current review surface reuses production `ScoreTypewriter` with deterministic UI-only Tier 1–5 controls and Replay so the updated key counts, 1.5×-faster typing cadence, physical depression, LEDs, smoke, and flames can all be checked while resizing the viewport. The former coffee-cup, money-ledger, money-total-motion, hidden-pattern, and word-score-transfer previews remain covered by automatic regressions.

> **Why "overwrite"? — resolving the double-counting problem.** Committing sentence bonuses per phase or per segment creates double-counting/cancellation problems. Instead, keep committed word score separate, re-score the current hole-delimited candidates, and project only the greatest single candidate gain. Re-judgment cost is negligible (short sequences). Fully compatible with variable phases: whatever the phase count, only the winning candidate at blind end matters.

> **Rainbow key lighting override (changed 2026-09-02; panel order changed 2026-09-03).** The 101 physical caps have immutable six-slot rainbow assignments. Every actually pressed key emits its own hard-edged pixel LED into the surrounding gaps; idle and unpressed keys emit none. Tiers 4–5 deterministically include all six colours and add one timing-synchronised flame on every pressed key, replacing the former single central flame. Tier 5 flames are 25% larger and may throw one ember pixel; Tier 3 keeps machine smoke, and Tiers 4–5 keep smoke and POP. The keyboard image's red/yellow/green panel indicators also blink on Tier 4–5; their three staggered phases use a deterministic random-looking order per beat/cycle, while Tiers 0–3 stay dark. The former full-keyboard rainbow ring and jackpot sparks are retired. These LED/flame colours are an explicit exception to §13: they remain fully coloured under `world-mono` and at every chromatic-unlock state. Reduced Motion removes moving LED/flame VFX while retaining low-intensity panel LEDs plus the existing static pressed/Enter accent, and Forced Colors hides the decorative LED/flame. This lighting override itself changes no counts, sampled-key mapping, repeat lifecycle, audio, or settle timing; the later 2026-09-07 Score Keyboard tuning above controls the current visual counts and cadence.

> **Will Score Keyboard exception (changed 2026-09-09).** Will keeps its final Chips ×0.5 and Mult ×0.5 engine beat and the ordinary score-event order. Its submission suppresses intermediate Score Keyboard beats, then starts one Break-primary beat from the final post-Will axes at the beginning of the existing 650ms final tally hold. Reduced Motion classifies the same final axes. No timer is added to `settleDurationMs()`.
...
- **Trigger.** After a submission's **full settle sequence** (word settle → Word-Hand/suit stamps → **sentence-finalize animation**: a finalized pattern receives the central callout while its provenance visibly lands on the score), if the total ≥ target the blind auto-resolves to **Fee Settlement** — the round number rolls up, then after a short verdict beat the settlement modal opens (there is **no** intermediate "Cleared! + Settle button" screen; item 4 removed it — the modal's own Collect button confirms). There is no cash-out fake choice: it never offers to continue past target, so surplus score stays worthless and remaining-phase gold still rewards a fast clear. The sentence bonus must be *seen* pushing the score over when it is the deciding factor — this is the game's highlight moment, so the beat lets it land before the modal covers the board. The finalized provenance remains readable behind the live modal until Collect; keeping it visible never extends the settle gate.
- **Target-cross cue (sample mapping changed 2026-09-02).** The Score Keyboard strikes its main Enter key once per blind and plays Enter's mapped WAV at the existing `deskEnter` accent gain on the exact live word/sentence count-up crossing, gated by the authoritative post-boss `projectedScore ≥ target` state; sample playback failure falls back to the synthesized `deskEnter` bottom-out/return. This logical cue is presentation-only and independent of the submission-local peak. After settle completion, a qualifying clear celebration may intentionally reuse the same mapped Enter WAV and fallback on each repeated Enter-first cycle; that does not retrigger the once-per-blind crossing event. Resolution still waits for the authoritative settle-complete signal plus the verdict beat.
- **Final-score authority.** Every sentence-scoring hook, including Broken Sentence when a real candidate has no pattern, is included in that candidate's live gain before the end trigger is tested. A history containing only holes or boss-removed submissions has no candidate and receives no sentence hook. Once the sentence-finalize sequence publishes `finalScore`, that exact value—not a later recomputation—is the sole input to clear/Game Over resolution. Temporary Emoji Tile firing VFX stop before either result modal appears, while the inert finalized provenance snapshot remains through Fee Settlement and is cleared by Collect.
...
Every pattern owns a base **[Chips × Mult]** (§5.2). For each candidate, raw pattern/modifier/Unison Chips first add to that candidate's settled word subtotal, a mixed-register factor then applies ×Chips to that combined value, and the factor gain is materialized into `sentenceChips` before hooks. The Mult side subsequently multiplies the combined result. Final blind score is `committedScore + max(candidateTotal − segmentWordScore)`. There is no per-pattern operation split; every pattern uses the same axis rule.

> **Balance warning — high-Mult sentences.** Sentence Mult amplifies only the settled word subtotal inside its candidate plus pattern/modifier/Standard-Unison Chips. High-Mult patterns therefore reward building a strong coherent segment without multiplying unrelated words across a gibberish boundary; target curves and pattern levels must still be verified together.
...
“contains/includes” fires once even when several tags qualify. Current-total
effects are also aggregate: Pouch Tag, Scrap Dealer, and Out of Print each apply
their complete displayed current Chips/Mult as one score event and one trigger
presentation, regardless of how many pouch steps, Brass tiles, or extinct letters
compose that total. Stored growth is
...
**Conditional-growth timing (changed 2026-09-04).** Every conditional growth or
decay presentation lands immediately after the exact committed event that
qualifies it, before any unrelated beat. When Glass is actually destroyed, its
material/result beat is followed by every applicable destruction-growth Emoji
Tile in physical shelf order before play advances to another tile or effect.
This presentation order never advances mechanical application: Term Insurance
and Type Foundry still snapshot their pre-destruction values for the shattering
word, and their new values begin with later words. Survival, prevented or
duplicate destruction, and debuffed submissions emit no destruction growth.

**Each phase:** submit word → settle & accumulate individual score (letter × suit multiplier × emoji tiles) while the displayed round holds at its pre-word committed value and the ScoreBox replays that word's Chips/Mult → split the eligible history at gibberish holes, independently score every non-empty sentence candidate, and project the greatest actual gain → display that winning candidate's pattern → once the full settle sequence has played, hold the authoritative committed gain in the always-reserved 52px status slot for 500ms, then transfer it into the round total over the existing 700ms count-up → if projected ≥ target and a real sentence bonus exists, keep sentence data absent and Chips × Mult at 0 × 0 until that word transfer finishes, then run BUILD and LAND as separate 700ms stages while the axes remain 0 × 0; after LAND and a short beat, auto-resolve to Fee Settlement (§7.2 — no early-end button, no intermediate verdict screen).

**On ending (early/final):** finalize only the winning candidate's gain — add
pattern/modifier/Unison Chips to its own word subtotal, apply one mixed-register
×Chips factor when active, then apply hook Chips and pattern/Unison/hook Mult
per §5.2–§5.3; every hole and losing candidate retains only its committed word score → grant the
...
cannot grant or revoke completion. Run Info shows the localized current Challenge
in a conditional full-width gold pixel banner between its header and tabs; normal
runs reserve no banner space. The Published screen adds a compact Challenge
Complete line when the profile is eligible to record it (§12.1).

**Endless curve (implemented 2026-07-31; number presentation changed 2026-09-04).** For Chapter `a ≥ 9`, let `c = a − 8`. The base target is `105000 × (1.6 + (0.75c)^(1+0.2c))^c`, truncated downward to two significant digits before blind, Pouch, Record, and boss multipliers apply. This double-exponential curve deliberately outruns slot-limited scaling. Chapter 38 is the explicit finite-number endpoint: clearing its Deadline ends the endless run; Chapter 39 is never created. Shared score/money text rounds once to integers, comma-formats rounded absolute values below 10,000,000, and otherwise uses the largest `M/B/T/Qa/Qi` unit with at most one decimal truncated toward zero; values from `1e21` retain compact scientific notation, and non-finite values show an em dash. The main ScoreBox keeps Chips and Mult as their authoritative independent axes and sizes each formatted visible string without recomputing the values: up to 7 characters uses 52px, 8–9 uses 38px, and 10+ uses 30px. Profile statistics track the best finalized blind score across every mode; Lifetime statistics separately track the highest Endless Chapter and best finalized Endless blind score.
...
| Uncommon Tag · 고급 태그 | Replace one item in the next shop with a free **Uncommon Emoji Tile** |
| Rare Tag · 레어 태그 | Replace one item in the next shop with a free **Rare Emoji Tile** |
...
Scarlet Tag is the sole presentation exception: its disclosed-letter icon remains
tooltipped and interactive throughout the played blind, then disappears when the
blind exits to Fee Settlement or Game Over. Other Tags do not reappear on later
screens; the source card's Tag remains non-interactive.
The tuning target remains a 20–35% skip rate.

### 8.3 Boss Pool — Design Principles & 18 Regular Bosses
...
| Will · 유서 (`will`) | One final boss beat halves Chips **and** Mult (×0.5 each); intermediate Score Keyboard beats are suppressed, then one beat starts from the final post-Will axes during the existing final tally hold; target-cross presentation waits for the resulting authoritative clear state | Attacks every build's base output; rewards patterns/multipliers |
...
| Family Photo · 가족 사진 (`familyPhoto`) | Adjective- or adverb-POS words score 0 (debuffed) | Pressures modifier-heavy vocabulary |
| No Smoking Sign · 금연 표지판 (`noSmokingSign`) | Noun-POS words score 0 (debuffed) | Pressures noun-heavy sentence skeletons |
| Wi-Fi Zone · 와이파이 존 (`wifiZone`) | Preposition- or conjunction-POS words score 0 (debuffed) | Pressures connective vocabulary |
| Transparent Paper · 투명한 종이 (`transparentPaper`) | Interjection-POS words score 0 (debuffed) | Pressures interjection plays |
...
uniformly to Forbidden Paper, Memoirs, Burnt Paper, Family Photo, No Smoking
Sign, Wi-Fi Zone, Transparent Paper, White Paper, and Dead Letter;
...
**Expanded 2026-09-09 to a seven-boss tier.** A finisher is drawn only for the Deadline of every Chapter divisible by 8 (8, 16, 24, 32). All other Deadlines use the 18-boss pool in §8.3. Stereotype Plate moved from the regular pool so its escalating word-length restriction is reserved for these periodic build checks. Each finisher pays **$8** instead of the ordinary boss reward. The draw and every in-blind random choice use the run's seeded RNG.

| Finisher | Effect | Presentation / counterplay |
|---|---|---|
| Stereotype Plate · 스테레오타입 판 (`stereotypePlate`) | Only hands at least as long as the longest valid word already played this Chapter may be submitted. | Sidebar, Blind Select, Boss Intro, and Run Info show that numeric current minimum, or no current restriction at 0; Collection stays generic. Keep the Chapter's earlier maximum manageable, then maintain or exceed it. |
...
| Nokdo Script · 녹도 문자 (`nokdoScript`) | One random letter tile must remain selected and must be included in the next submitted word. It cannot be deselected or discarded. Consumables may still transform it—including changing it to Stone—or destroy it. If it leaves the hand, another random hand tile becomes forced; an empty hand has none. Magnifier hints reserve this exact physical tile when it still carries a letter. If it is Stone/letterless, the advisory hint instead spells from the remaining letters and omits the Stone from its displayed tile ids; submitting the forced Stone with those tiles is still gibberish unless an existing effect such as Stone Tongue changes spelling. | A parchment bearing deer-track-like glyphs; build or consume around the forced tile. |
...
**No-repeat cycles (pool sizes changed 2026-09-09).** The 18 ordinary bosses and seven
...
**Pool intent:** the 18 regular bosses cover each system roughly once, while the
seven finishers are periodic build checks. Memoirs remains scoped to the chapter;
the old Proofreader/Babel finishers remain retired.

**Debuff convention (changed 2026-09-10).** "Debuffed" (including boss and Lipogram Tag predicates) means an allowed physical play whose ordinary scoring pipeline stops immediately after pure word preparation/rule lookup. It normally produces one 0-point settle and never runs tile/material/font/edition scoring, Word Hands, played/held/owned Emoji Tile scoring hooks, `wordChecked`, `tilesPlayed`, `wordScored`, Briefcase balancing, score bonuses, score-side chance RNG, or score/progress mutations. **Uncensored is the sole exception:** a debuffed dictionary word emits one Uncensored +100 Chips event and settles at `100 × 1`; every other short-circuit restriction remains in force. The submission keeps `posUsed = null` and is removed before candidate splitting and Pattern, Unison, and register-synergy judgment, so normal words before and after it become adjacent. It remains visible as a disabled tray record; physical tile/phase consumption, draw, boss `afterPlay`, actual-play Collection/chromatic/mascot discovery, and `wordsThisAnte` remain intact. Gibberish is unchanged at layer 1: it scores its normal payout and becomes the boundary between independent sentence candidates.
...
Balatro-mirrored baseline: **Item slots ×2** + **Pack slots ×2** + **Voucher slot ×1**. The first Stationery Shop actually entered in a run guarantees one **Basic Charm Pack**; skipped blinds do not consume this guarantee. Voucher Tag may add one temporary second Voucher choice. Each rarity Tag instead replaces one ordinary item-slot offer with its guaranteed free Emoji Tile, so it never increases the current item-slot count; the replacement remains in that slot across rerolls. Available item-family weights are **Emoji Tile 20 · Fable 4 · Constellation 4** (`balance.ts` `shop.itemWeights`). EN-KO Dictionary adds letter tiles at weight **4**; Encyclopedia lets those shop tiles roll material and edition, but no font. **Lucky Pouch** adds implemented Gambler Cards as a separate weight-**2** family (§12.2). Unavailable families are removed and the remaining weights normalize automatically. Story Book/Novel and Bible/The Law multiply the corresponding base weight by **×2/×8**. **Reroll:** base 5 Fee, +1 per reroll, refreshes item slots only; packs and the Voucher are unchanged. Reroll Tag changes that next visited shop's base to 0, producing $0, $1, $2… before ordinary discounts/floors.

**Offer interaction (action width corrected 2026-09-03).** Shop stalls are image-first. Emoji Tiles, consumables, and the vertical voucher use the shared rounded `124×165px` stage. Sale packs use the requested older `131×229px` foreground with square corners. Their row slots match the 131px art width, preserving the normal 12px gap between packs, and the pack panel reserves enough lower space for the attached Open button to remain inside the persistent run layer. The price tag shares one foreground layer with the product and action. Selecting an offer raises that complete layer by 59px — the 15px base lift plus the 44px action-button height — and reveals the attached action: **Buy** for ordinary stock, **Redeem** for the voucher, and **Open** for packs. Those primary actions keep their original 82px minimum width regardless of the larger global blue-button rule. This does not reflow the stall layout. When an instant-use option exists, Buy remains below while **Use now** appears vertically centred outside the product's right edge. Product animation is never clipped. Voucher and pack background panels retain a `273px` minimum height. Only one offer action is expanded at a time; sold stalls render as empty placeholders.

**Shop rail localization containment (full-label wrapping changed 2026-09-09).** Next Blind and Reroll remain equal-sized inside the fixed 168px rail. Overlong localized labels wrap in full with reduced horizontal padding and last-resort word breaking instead of widening into the sale layer or showing an ellipsis.

**Persistent framing (changed 2026-08-30).** The shop is a lower panel on the same run table as the blind. The sidebar resets score/Chips/Mult/hand/discard readouts and displays SHOP; owned Emoji Tiles, consumables and the pouch remain mounted. On the shop's first frame the pouch switches from the completed blind's undrawn remainder to the complete permanent `run.bag`; stale `blind.bag` contents must never remain visible there. The finalized sentence provenance intentionally survives the live Fee Settlement and is consumed only when Collect performs the same-transition Shop reset. That transition also consumes the previous blind's UI-only settle log/id and finalized sentence fields before the first Shop frame; the zero reset is immediate and must never replay the prior score animation.
...
**Money-gaining Fable Use Now ledger (changed 2026-09-04).** A successful Stationery Shop **Use now** of The Goose That Laid the Golden Eggs or The Heavenly Maiden remains one atomic purchase/use transaction, but its presentation never collapses to a net amount. It shows the nonzero signed price beat first (`−$price`), then the nonzero full Fable payout (`+$payout`), while the shared result vignette continues to show that same full payout once. Goose still calculates its capped gain from the post-price balance, and Heavenly Maiden still uses its existing capped total Charm sell value. A zero price or zero payout omits only that beat. The persistent Sidebar is the sole Shop balance and renders and announces one portalled ledger only while the Shop is active; leaving the Shop immediately hides and clears active or queued ledger presentation. Final gold, affordability and other preconditions, offer removal, consumption, counters, RNG, save data, and audio are unchanged.

**Displayed money motion (changed 2026-09-04).** Every distinct displayed balance change updates the formatted total immediately and restarts a 480ms transform-only motion on the nested total text: gains peak at `translateY(-4px)` with slight squash/stretch and losses mirror at `translateY(4px)`. Initial/unchanged values and both Reduced Motion paths do not move. Ordinary signed popups remain; a Shop ledger transaction suppresses only its ordinary net popup, so the total still moves once by the atomic net direction while the ordered price/payout ledger remains authoritative.
...
| Sketch Book → Portrait | One boss reroll per chapter for $10 → unlimited $10 rerolls; the control is available only on Blind Select when the current blind is the Deadline, never while it is merely upcoming | Discover all 18 regular bosses (finishers do not substitute) |
...
blinds** — essential: Magnifier and tile-targeting effects only matter mid-blind. Acquired
...
**Constellation Pack resolution (changed 2026-07-29).** A revealed Constellation follows the same select-then-confirm interaction, but its action is always named **Use**, never Select. Use immediately levels the mapped sentence pattern, plays the full Constellation level-up sequence, ignores held-consumable capacity, and does not place the card in a held slot. The 3.5-second sequence and the 2.4-second shared consumable result vignette are modal interactions: focus moves inside and remains trapped, background input and Escape/game shortcuts are blocked, inner tooltip objects remain available, and connected prior focus is restored when the presentation ends. They remain body portals rather than native top-layer dialogs so shared body-portalled tooltips stay visible.
...
the complete permanent `run.bag` in the Shop. Scoring applies and presents that
complete current payout once, rather than replaying one +5 beat per pouch step.
...
Plate rolls fail; Cubism grows once whenever either roll succeeds, including retriggers. Alphabet
...
| C54 | Cubism · 입체주의 | Starts at ×1; whenever a Lead Plate tile effect succeeds, including a retrigger, add +0.25 to its factor | 1 | ★ ×Mult |
...
| U22 | Scrap Dealer · 고물상 | Add +0.2 Mult for each Brass tile in the permanent pouch; scoring and the live current tooltip (including +0) share one headless total helper, and the complete current Mult applies as one score event | 1 | dynamic +Mult |
...
| R4 | Out of Print | Gain +50 Chips and +8 Mult for each alphabet letter with no copies left in the permanent pouch; its displayed current totals apply as one score event | 1 | dynamic · Remove every copy of one letter |
...
| R44 | Term Insurance · 단기 보험 | Each actually destroyed letter tile gains +0.2 ×Mult, with no destruction prevention | 1 | ×Mult |
...
- **Emoji Tile scoring order (changed 2026-09-03):** during the global owned-list `wordScoring` stage, each physical owner resolves its Gray/Violet/Rainbow edition before its own intrinsic scoring hook and growth/decay beats, then play advances left-to-right. Base/White emit no scoring-edition beat. Per-tile Emoji triggers retain their earlier played-tile phase; an owner edition never moves ahead of those tile-bound triggers.
...
`BALANCE` values above. During an active Challenge, Run Info presents its localized
name in the conditional CSS-only banner defined in §8.2.

**Start/Continue disclosure (changed 2026-09-04).** The public New Run modal has
only New Run and Continue tabs; the Challenge-start tab and its panel are created
only in DEV builds, and its implementation chunk is excluded from production.
This presentation restriction does not retire Challenges:
production continues to validate, load, display, and resume a saved run carrying
a known `challengeId`, and all in-run progress/completion rules below remain.
Every tab shares one compact fixed `720×700px` border-box shell with a scrolling content viewport,
fixed primary-action and reserved-note rows, and one full-width Back control.
Continue presents the saved Pouch and Record plus committed score/target, Fee/돈,
best word, seed, and optional localized Challenge name; projected/final score is
never substituted for the committed value. Its real Pouch/Record objects use the
same `140×140px` / `88×88px` desktop and `92×92px` / `68×68px` narrow sizing as
the New Run selectors.
...
Do the same **independently** for the winning candidate's sentence contribution axes after the
register-synergy ×Chips gain has been materialized and pattern, modifier,
Unison, Emoji Tile, voucher, and boss hooks have resolved, immediately before applying
those axes to that candidate's settled word subtotal. Do not include words outside
the candidate in the mean. Keep the arithmetic mean at full precision—no floor, ceiling, or nearest
...
The playable world begins **desaturated and its gameplay mixer silent**; playing specific words naturally unlocks presentation layers. The limited Settings grant described in C-4 is the sole direct Palette exception. The pre-game Sweet Turtles developer ident is fully separate from that mixer (C-6). Its physical-viewport backdrop is literal `#000`, it stacks above the global CRT layer, and its mark always renders in its original colours with `filter: none`; profile colour unlocks and `#unlock-chroma` never affect it. This is the literal enactment of the title — you *play the world into existence*. Persistent **per profile** (localStorage `wj.unlocks`, beside collection/tutorial flags). **Valid words only** unlock naturally (gibberish never does).
...
The immediate `ChromaticReveal` remains for natural word unlocks, and the same newly played Palette ids
...
terminal screen. Reveal All and Settings' synthetic required-Palette grants are
absorbed into the baseline and never presented as play-earned unlocks. Settings'
grant instead fires one atomic aggregate reveal after both profile and active/saved-run
baseline writes; it never queues six individual word reveals.
...
| YELLOW | gold token group — money, gold UI, target-cross/score-keyboard accents |
...
Collection's consumable-family buttons follow the same token groups: **Fable = RED**
...
**Settings audio exception (changed 2026-09-03).** The C-4 required-Palette grant
also enables MUSIC/SOUND immediately; the existing per-bus mute and volume values
remain authoritative. This is the only alternative to natural word play for these
two gates. It triggers one aggregate Palette reveal/fanfare, never their individual
word reveals or run-end recap entries.
...
**Profile unlock-all and Palette Convenience grant (C-4, changed 2026-09-03).** The Profile screen owns the
broader **Reveal All / 모두 잠금 해제** escape hatch; there is no device-wide override.
Settings → Game has one narrower current-profile **Palette Convenience / 팔레트 편의성** action derived from the registry's color/audio rows
(currently RED/YELLOW/GREEN/BLUE/MUSIC/SOUND). Its first press changes no state and shows an inline
irreversible warning; the same button's second press merges only the missing six ids into `wj.unlocks`,
preserving mascot and unknown ids. It changes no Collection, Emoji Tile, Voucher, Pouch, Record,
Challenge, tutorial, or machine setting progress. The second press first persists the profile grant,
then durably absorbs those synthetic ids into any active or saved run's terminal-recap baseline, then
emits exactly one aggregate `unlockBus` event containing only the actually added required definitions.
`ChromaticReveal` applies presentation and one fanfare using the existing mute/volume values and 2.6-second
click-dismissible/Reduced-Motion treatment. Its `document.body` portal sits above the in-run pause overlay
and below tooltips so the confirmation reveal cannot expire behind Settings; it displays only `The world has been restored` /
`세상을 되찾았습니다`. Six individual reveals are forbidden. Tab change, Back, and unmount disarm confirmation. The Reveal
All first press changes no unlock or discovery state and only persists that the selected profile saw the
warning. A later press fills that profile's
...
`paletteGuide.unlock` template with the target word, such as `“RED”로 해금`; per-word
explanatory sentences are retired. A submission that unlocks one
of those rows, or ends the blind, consumes that blind without a reminder. Pause,
Run Info, Boss Intro, blind-entry effects, coach-marks, chromatic reveal, and every
non-playing phase suppress it. The whole top-right viewport notice is a button
whose width shrink-wraps its localized text and close mark, with a viewport-safe
...
falsely promising that yellow will return.
