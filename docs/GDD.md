...
> **Rotating developer Laboratory (changed 2026-09-04).** The DEV-only Main Menu Laboratory is not a permanent feature gallery. Whenever a completed player-facing implementation or change cannot be inspected immediately in Collection or Run Info, its current contents are replaced by that one review surface; work already reachable there and internal-only refactors do not enter the Laboratory, and automatic tests retain coverage for retired previews. It reuses production components and data rather than copying them, and never constructs or mutates engine/RNG/run/discovery/profile/settings/storage state. Its current review surface replays the real Shop Use Now money-ledger renderer with fixed `$10 → −$3 → +$7 → $14` demo data. The former hidden-pattern, Score Feedback, and Desk Encounters previews are retired; their real run behavior and automatic regressions remain unchanged.
