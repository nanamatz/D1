...
> **Score Keyboard / 점수 키보드 (progressive local tier + repeated clear celebration changed 2026-08-30).** A solid fictional vintage terminal keyboard stays idle in the viewport's left margin and reacts only during score resolution; runtime random desk encounters remain right-only. Its locally bundled **1774×887 transparent RGBA master** is a natural 2:1 landscape object with three flat key wells and no baked caps or legends. The complete raster + DOM-key wrapper rotates 90° counter-clockwise into a vertical 1:2 dock: width is `min(230px, 13vw, gutter − 16px, (100vh − 32px) / 2)`, height is `width × 2`, it is vertically centred, and `object-fit: contain` may never crop, stretch, filter, or change object opacity. One UI-only formula/row registry supplies **101 fully opaque, labelled, disabled, untabbable ANSI-derived caps**: ESC, F1–F12, PRINT/SCROLL/BREAK; the standard main block without Windows keys; ten navigation/arrow keys; and a 17-key numpad. A–Z appears exactly once. Authentic proportions include 2u Backspace, 1.5u Tab, 1.75u Caps, 2.25u Enter, 2.25u/2.75u shifts, 7u Space, 2u-wide numpad 0, and 2u-tall numpad +/Enter. Every cap belongs to the deterministic candidate pool. Each `settleId` starts at Tier 0. Immediately after every score-increasing beat, the current submission-local subtotal is `max(0, Chips × Mult + flat score)`; divide it by the positive finite `blind.target` for raw Idle/Tier 1–5 at `r ≤ 1`, `1 < r < 1.25`, `1.25 ≤ r < 2`, `2 ≤ r < 4`, `4 ≤ r < 10`, and `r ≥ 10`, then present `max(previous peak, raw)`. A decrease, zero-score, gold-only/stat-only beat, or invalid value cannot lower or falsely raise that peak. `committedBefore`, earlier submissions, round projection, and sentence BUILD/LAND are excluded from the numerator. Every eligible beat uses its current peak and includes one semantic primary before deterministic fill: tile/tile-bound beats use their submitted A–Z key, letterless Stone uses Space, suit/word-length/Word-Hand/global use main Enter, untargeted Emoji Tiles use stable F1–F12, boss uses Break, Tag uses Tab, Pouch uses Space, and unknown uses Enter. Tiers retain 3/5/8/12/16 visible caps and 1/2/3/4/5 audible presses, with the primary audible first and every release inside the snapshotted 1×/2× beat. Normal submissions return to idle on settle completion. For an actual clear with a non-zero peak, settle completion immediately starts synthetic `clear:{blindKey}:{settleId}:{cycle}` score beats through sentence BUILD/LAND, final count-up, and verdict; Fee Settlement, a terminal modal, or the next blind cancels them in the same render. Main Enter is always the primary and first audible slot (`deskEnter`), with deterministic key fill and ordinary `deskKeycap` sounds completing the tier's 1/2/3/4/5 audible count. The repeat interval is the 460ms score-keyboard beat times `[2, 1.75, 1.5, 1.25, 1]` for Tiers 1–5, divided by the clearing submission's snapshotted 1×/2× speed (920/805/690/575/460ms or 460/402.5/345/287.5/230ms). Every cycle replays tier-scaled local body/key motion; Tier 4 adds smoke and Tier 5 adds smoke, flame, and POP, while tier light remains between beats. Tier-0 clear, loss, sentence-only, and non-clearing submissions never loop. Crossing the cumulative live target is still one logical once-per-blind main-Enter event; its central synthesized `deskEnter` sample is intentionally reused by each later clear-celebration cycle. Reduced Motion before or during the hold synchronously cancels repeat timers, audio, and moving VFX, retains only the static peak/Enter accent, and preserves the existing one-shot target-cross audio exception; turning it off applies from the next submission. These effects add no timer to `settleDurationMs()`, never trigger global screen shake, never step the committed-only round number, and never bypass settle-complete.
