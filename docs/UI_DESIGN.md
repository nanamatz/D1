...
| `--gold` | `#F0B23E` | money, current pattern, target-cross/typewriter accent | **YELLOW** (locked `#A89E84`) |
...
- **Startup developer ident (changed 2026-08-29).** Before the real Loading screen, show only the bundled Sweet Turtles mark—no publisher beat—as a two-sided `300px` coin capped at `62vmin`. Its front is the true circular, centred `object-fit: cover` logo; its neutral metal back carries raised `ST`, never mirrored logo art or text. The ident covers the physical viewport with literal `#000` above the global CRT layer, independent of palette/background tokens. The automatic 3850ms beat needs no input: 0–1000ms drops from `-34vmin`, fades in over 120ms, and rotates vertically on `rotateX` from `0→1260deg` while scaling `.70→1.04`; contact compresses the coin and expands its shadow; 1000–1220ms bounces `-18px` and turns `1260→1440deg`; 1220–1450ms relands/decelerates to `1710deg`; 1450–1600ms snaps to exactly `1800deg` (five turns, front visible); 1600–3600ms locks the front for exactly two seconds, plays one ring shine, and reveals the Sweet Turtles name; 3600–3850ms fades the whole ident out. The cue combines one bright metallic ding at the initial appearance, deterministic spin ticks, contact/body impact, bounce clink, and final metallic shine through a fixed peak-safe output wholly separate from every game mixer setting, mute, and Palette audio gate. Web playback is one best-effort ungestured attempt and never queues for a later gesture; Electron enables that attempt before window creation. Either reduced-motion path instead runs 2240ms with no drop, flip, bounce, shadow, or shine: a static front fades in for 120ms, stays fully visible for exactly two seconds, and fades out for 120ms while one small metallic brand chime plays. The beat starts only after the local image loads and decodes; preparation shows only the black backdrop, while an error or two-second timeout remains a separate silent 700ms text-only fallback and permanently suppresses the late image. It is unskippable, has no focusable control or input listener, and never adds fake loading progress or persistence. Because it sits outside `.frame`, it owns an explicit first-paint greyscale gate for profiles with no colour unlock; after any colour unlock the original logo colour may show.
...
2. **Pattern update**: after settle, the current-pattern symbol and name re-evaluate with a soft flip. Projected score updates internally. Reaching the target is signalled separately by the Score Typewriter's one-shot bell/punch while scoring continues toward the settle-complete gate (GDD §7.2); it never changes the typewriter's strength tier.
...
7. **Score Typewriter (unlabeled authored caps revised 2026-08-29):** a non-interactive typewriter is fixed in the viewport's left margin and remains visibly idle throughout every in-run phase. It is an object-only image—no desk, props, background, frame, halo, or paper. The locally bundled **320×1760 transparent RGBA pixel-art master is a body-only chassis with no baked keycaps**, showing one deliberately elongated mechanical typewriter rotated 90° counter-clockwise. Its limited palette, hard edges, and restrained dull-gold/red accents match the game screen; bright ivory, polished-metal gradients, antialiasing, and micro-detail are excluded. The dock begins at 3vh, occupies 94vh and the safe available lane width, and intentionally uses `object-fit: fill`: whole-panel coverage takes priority over mechanically exact proportions, while the board boundary remains inviolate. The raster alone is filtered and deliberately low-salience (`0.52` opacity, `0.62` while active; brightness `.72`, saturation `.55`, contrast `.9`) relative to the central score HUD; overlays retain their ordinary value. Its dock sits above ordinary board/desk surfaces but below transitions, modals, tutorial and tooltips. An invisible positioning plane holds **27 unlabeled native DOM buttons** authored one by one with x/y position, scale, and tilt. Their loose 10/9/8 curved mechanical banks use no row/group DOM, visible legend, grid, border, or panel background. Every cap is disabled, untabbable and hidden from accessibility APIs because the machine is feedback, not input. Idle caps are round, slightly squashed navy/charcoal mechanisms with muted metal rims, one-pixel highlights, and hard bottom/right shadows—not gold UI controls. A 32-bit hash of the beat id creates its deterministic unique permutation, and `keyRhythmJitter = 0.35` creates uneven normalized gaps without engine RNG or `Math.random()`. During score resolution, each selected cap itself travels inward 2–3px, scales slightly, collapses its shadow, and gains only a subtle dull-gold face; containers never animate. Tiers press 3/5/8/12/16 unique visible caps while key SFX retain their quieter 1/2/3/4/5 density at matching press times. Carriage travel, local shake, smoke, Tier-5 flame/POP, target DING, tier formula, sentence finalization, snapshotted speed, and settle gate retain their existing contracts. Reduced Motion cancels key SFX and all key/carriage/body/shake/smoke/flame/POP motion and leaves only low-intensity static metal accents on the selected caps, never a rectangular keywell outline. Because the typewriter is portalled outside the zoomed/filtered frame, `world-mono` explicitly greyscales the raster and every overlay until a colour unlock lands.
