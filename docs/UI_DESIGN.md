...
- **Placement (shipped; balance panel removed 2026-09-04):** bottom-aligned below the Next Blind/Reroll controls in the existing 168px shop rail (proprietor behind the counter feel), with no separate local balance panel and never overlapping the item slots. Hidden on the ≤720px single-column layout, lying on a pink **cat-face plush cushion** (`docs/Arts/Mascots/T_Cushion.png` → `src/ui/assets/piyak-cushion.png`) — the plush sits behind him, ears peeking above his back. (The earlier in-house generated pillow is retired.)
