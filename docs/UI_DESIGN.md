...
**Text-selection suppression (changed 2026-09-09):** game surfaces and body-portalled overlays use `user-select: none` so pointer dragging never paints the browser's blue selection highlight. Inputs, textareas, and editable content restore native text selection.

**Back action icon (changed 2026-09-09):** Back buttons keep arrow glyphs out of localized text and prepend one shared decorative, hard-edged pixel left-arrow icon. The localized label alone supplies the accessible name.
...
**Guided Palette Convenience spotlight (changed 2026-09-10):** YELLOW settlement releases the lesson lock. After Fee Settlement is collected and the first Shop is visible, reuse the real in-run Settings > Game surface and body-portalled coach-mark to spotlight its Palette Convenience button. The overlay blocks the button itself; Next or Skip returns to the Shop without granting unlocks, then the Shop first-visit guide opens.
...
- **Pixel grid.** Author raster art at fixed pixel resolutions and render it with `image-rendering: pixelated`. Every shell-transition canvas—Main Menu, New Run, Run, Collection, Options, Profile, and Laboratory—shares the `1440×988` outer board (`--board-max` / `--board-h`) and responsive viewport-fit zoom plus the Settings UI-scale slider. Screens use that common minimum-height canvas while their existing inner content widths remain constrained; short viewports fit-scale and long content scrolls normally. This prevents the outgoing/incoming slide from clipping against mismatched screen heights. During a transition the incoming pane remains in final viewport coordinates and reveals itself with a moving clip while only its content translates, so the shared desk background never repaints in a moving or shorter coordinate space in fullscreen. The scale is intentionally not quantized to integer steps, avoiding excessive letterboxing or an undersized board. Borders/shadows are 1–2 "big pixels" wide, not sub-pixel. (fullscreen transition background corrected 2026-09-09)
- **New Run shell (changed 2026-09-04).** One compact framed `720×700px` border-box modal keeps the `620px` tablist, internally scrolling content, `82px` centred primary-action row (action maximum `340px`), always-reserved optional-note row, and full-width Back in fixed slots. Seed and Challenge notes never reflow the actions. Production creates only New Run and Continue tabs; DEV appends Challenge start from a DEV-only dynamic chunk that is absent from production. Its selected Challenge detail omits the redundant Challenge name/description and shows only the loadout and its effects, avoiding an ordinary content scrollbar. A known saved Challenge still resumes in production and places its localized name in the reserved note. Continue reuses the real tooltipped Pouch/Record objects and their exact New Run sizes (`140×140px` / `88×88px` desktop, `92×92px` / `68×68px` at `≤620px`), and shows only committed score against target, never projection/final score, with economy labelled Fee/돈. Forced Colors preserves the frames; Reduced Motion changes no geometry.
- **Shell transition identity (changed 2026-09-10):** outgoing and incoming panes preserve the same content-wrapper/provider hierarchy. Capturing a New Run pane must not remount it or reset its selected tab to Continue before the slide begins.
- **Seeded-run action override (changed 2026-09-10):** the Seeded Run checkbox, its conditional input, and Play all live in the fixed action layer, never in the Pouch/Record content viewport. The checkbox shares the action layer's upper row with the input when enabled; enabling it expands only that layer from `82px` to `116px` and lowers Play without creating content scroll. This supersedes the fixed-`82px` sentence above only while the input is present.
...
- **Gameplay speed/audio settings (speed scope changed 2026-09-06):** Game offers the ordered accessible choices 0.5×, 1×, 2×, and 4× in one responsive segmented group; 1× is the default. It changes only the spacing between ordered score-calculation events; impact, feedback/VFX/audio/shake, Score Keyboard, final hold, score transfer, sentence resolution, modals, and transitions stay at fixed 1× timing. The note is exactly `Audio / 오디오`. Music and SFX share one four-column `label | slider | value | mute` grid: desktop `minmax(72px,1fr) minmax(140px,160px) 44px 76px` with 12px gaps, and ≤480px `64px minmax(96px,1fr) 36px 68px` with 8px gaps. Both native checkboxes visibly read `Mute / 음소거` but retain bus-qualified accessible names. There is no public Master control. A slider edit never unmutes, and unmute restores that slider's retained value. Gameplay gain is the Palette bus gate AND not-muted, then group volume × recipe gain. Legacy Master 80/Music 70/SFX 80 migrates once to Music 56/SFX 64; Master 0 preserves both stored groups and mutes them. New mute booleans make stale Master irrelevant, and normalized writes drop it. The startup ident remains outside this contract.
- **Native checkbox treatment (changed 2026-09-10):** every native checkbox keeps its semantic input but shares one square, hard-edged pixel/CRT frame with a gold check, visible keyboard focus, disabled state, and Forced Colors fallback.
...
- **Palette Guide setting (added 2026-09-09):** Game exposes a separate On/Off toggle, default On, for the ordinary required-Palette reminder. Off synchronously removes any visible reminder and suppresses future ones; it does not disable the guided introduction or first-encounter tutorial popups controlled by Tutorial Tips.
- **Settings tooltip placement (changed 2026-09-10):** every Game, Video, and Audio value tooltip prefers opening below its control. Palette Convenience uses its localized irreversible-warning copy for the tooltip as well as the armed inline warning.
- **Persistent sidebar height (changed 2026-09-04):** play, Shop, and Blind Select share one fixed `264px` top-badge footprint. Together with the common round panel and `484px` score/control allocation, this fills the same `956px` inner rail in every mode, so no screen leaves a shorter-looking content stack or bottom gap.
- **SHOP marquee (changed 2026-08-20):** the shared sidebar badge footprint and hit box stay fixed, while the light treatment runs around all four outer edges rather than occupying a single bottom row. Reduced motion keeps the complete perimeter visible and freezes only its blink cycle.
...
**Signature element — the Sentence Tray.** Balatro has no equivalent: our played words accumulate left-to-right as sentence candidates separated by gibberish holes. Each played word is a mini-card group (its tiles shrunk), framed independently in its suit color, with every lexical POS candidate rendered as its own localized chip beneath—never slash-joined. Every POS chip uses the same compact rectangular shape with no family marker, but all ten ids have distinct neutral tokens and distinct unlocked colours: noun/interjection belong to BLUE, the three verb subtypes to RED, adjective/adverb/article to YELLOW, and conjunction/preposition to GREEN. No two ids share their final colour. The localized label remains authoritative while locked neutral luminances preserve the distinction in `world-mono`. Once a sentence candidate wins, compatible candidates from the headless judgment are accented only inside that winning segment and alternatives are dimmed through their background/border—not container opacity; equivalent winning parses keep their union active. Resolved chips expose localized compatible/alternative accessibility labels without single-selection semantics, and forced-colors mode preserves the resolution-state distinction with solid versus dashed borders plus a non-layout-shifting active outline. With no pattern all candidates remain equal. Candidate rows wrap without overlap (up to seven tags). The staged-word preview reuses the same separate chips without inventing a selected POS. Gibberish keeps the cracked/burned **hole** treatment without a secondary hole/new-sentence label; debuffed words keep Not Allowed with no POS. At the tray's right end, a pattern status chip shows the live winning judgment ("Transitive ✓ ×2" / "— no pattern") and changes only when another segment overtakes it. This tray is the game's identity on screen; spend the polish budget here. (changed 2026-09-10)
...
**Palette reminder toast (width corrected 2026-09-09):** use a `document.body` portal so it stays pinned to the physical viewport's top-right outside the filtered/zoomed board. The visible message uses one compact localized `{word}` template (for example `“RED”로 해금`) rather than a full explanatory sentence. The pixel-panel button shrink-wraps its localized text and close mark instead of reserving a fixed width; only the viewport-safe maximum may wrap long translations. It remains inside a polite status live region. Enter from beyond the right edge in 240ms, hold six seconds, and leave right in 180ms; hover/focus pauses the hold. Both Reduced Motion sources remove transform/opacity animation and show/remove instantly. It is non-modal and never dims, blocks, or reflows the play board.
...
When the separate live-value row already includes an Emoji Tile's base value,
the effect plate omits that duplicate base performance and shows only its rule,
decay, or condition explanation. This applies to Beehive Tile, Dulling Pencil,
Drying Ink, Folding Manuscript, Shuriken, and Word Hunter. Non-dynamic standalone
values may still occupy their own first line. The same localized copy is shared
by the Shop, owned shelf, opened packs, and Collection. (changed 2026-09-09)
...
Pouch Tag, Scrap Dealer, and Out of Print each animate their complete displayed
current Chips/Mult once when scoring. Their contributing pouch steps, Brass tiles,
or extinct letters never replay as separate Emoji Tile trigger beats. (changed 2026-09-09)
Peddler likewise shows and applies its complete current additive Mult from owned
Emoji Tile sell values once, including +0. (changed 2026-09-10)
Type Orchestra applies ×1.5 once for every distinct font, including Medium, when
at least two fonts are present; this supersedes the older score-sequence example
that said ×1.25 and excluded Medium. (changed 2026-09-10)
...
body colour outside the box. A Chips/Mult value and its unit are an atomic
line-breaking unit: combinations such as `+2 Mult`, `×2 Chips`, `+2 배수`,
and `×2 칩` move to the next line as a whole and never split internally.
Other semantic highlights retain last-resort wrapping for viewport containment.
(changed 2026-09-10)
...
supplement prefers the left of the main tooltip. With two or three, the
highest-priority one folds into the main effect plate and the rest prefer the left;
...
definitions stay together; if another definition exists, only the highest-priority
non-edition definition may fold. The stack moves right when only that side fits
and folds inside the main card when neither side fits; opening downward does not
otherwise change the distribution. Any tooltip effect copy that names a tile font
...
Descriptive copy keeps words intact where possible (`word-break: keep-all`) and
uses `overflow-wrap: anywhere` as a last-resort containment rule; no translated
token may escape its surface or the physical viewport. Pouch selection,
clicked-open Pouch descriptions, controls, and mascot dialogue use the same rule.
All tooltips are vertically viewport-contained. Object tooltips are pointer-transparent so moving the pointer off their object always closes them and the panel never blocks objects beneath it. Run Info pattern examples are the sole interactive exception because constrained viewports must scroll their uninterrupted example row. All tooltip copy uses the enlarged readability scale: 18px title, 15px body,
...
badge in every locale.
All effect/tooltip descriptions in every locale replace non-terminal periods
with line breaks, omit terminal periods, and contain no semicolons; decimal
...
Traditional Chinese uses Taiwan forms and terminology: 籌碼, 倍率, 盲注,
稿費, 字袋, 唱片, 句型, 語域, and 亂碼; UI terms use forms such
as 解析度, 滑鼠, 點擊, 影片, 音訊, 儲存, and 載入.
...
| `ScoreBox` | blue Chips box × red Mult box, Jersey 10 numerals (final-settlement display clarified 2026-09-06). Display precedence is finalized sentence bonus fixed at 0 × 0 → active word settle → staged Word-Hand preview → 0. During ordinary word settlement the axes replay the word's Chips and Mult beats. During final sentence settlement they remain 0 × 0, so the committed round score is never replayed as the Chips axis. A staged Hand uses its exact preview bonus Chips and Mult, including ×1; debuffed or handless previews return 0 × 0, and a hidden Hand keeps `???` only for its name while showing the numeric bonus. After authoritative settle completion, the always-reserved 52px status slot immediately above it temporarily shows the authoritative positive finite word gain with no plus sign while Chips × Mult already reads 0 × 0. The gain appears once with a 260ms centred scale pop (`.55 → 1.12 → 1`) without replaying as its value counts down. That neutral readout and the committed-only round total hold unchanged for 500ms at every game speed; then the gain decreases to zero as the round increases over the existing 700ms tween. The slot remains empty through BUILD so the ScoreBox never moves. Every status variant is vertically centred in that fixed slot; the enlarged Word-Hand label keeps a safe line box and top clearance while remaining one-line with ellipsis and Forced Colors support. The transfer uses `formatScore`, is decorative/`aria-hidden`, and never shows or waits for zero, negative, or non-finite values. Reduced Motion removes the pop, keeps the same static 500ms post-settle hold at 0 × 0, then removes the gain as the round snaps to committed without tweening. Both axes use shared `formatScore`: round once to integers; comma-format rounded absolute values below 10,000,000; from 10,000,000 use the largest `M/B/T/Qa/Qi` unit with at most one decimal truncated toward zero; retain compact scientific notation from `1e21`; and show an em dash for non-finite values. Each formatted string is reused once and independently classed by visible length: ≤7 characters uses 52px, 8–9 uses 38px, and ≥10 uses 30px. The inner numeral is centred, single-line, and clipped to its box while the outer box stays overflow-visible for contribution VFX. Accessible labels announce localized Chips and Mult with those exact formatted values. The actual axes are never redistributed. Semantic effect popups keep their own axis precision. |
...
Rapid transactions queue instead of replacing an unseen beat. The persistent
Sidebar is the sole Shop balance and owns the one portalled ledger/live region
only in Shop mode; leaving Shop immediately hides and clears active or queued
ledger presentation, while ordinary money pops remain unchanged elsewhere. All
signed text uses `formatScore`.

**Money total motion (changed 2026-09-04).** `MoneyValue` keeps its existing
signed popup and wraps only the formatted total text in an inline transform
target. Every distinct gain restarts a 480ms upward hop peaking at −4px with
slight squash/stretch; a loss mirrors it at +4px. Initial and unchanged values
stay still, both Reduced Motion paths update instantly without travel, and no
layout or accessible text is duplicated. Shop ledger suppression applies only
to the ordinary net popup: the total still performs one motion from the atomic
net delta while the price/payout beats retain their order.
...
| Polished | high-contrast mirror bevel, broad fixed diagonal buff band, stronger corner sparkles + faster stepped pixel shine sweep (shine strengthened 2026-09-04) | — (flat +Mult) |
...
| Ivory | neutral white-to-cool-gray face, silver-gray inset ridge and curved Schreger-line grain (changed 2026-09-04) | — |
...
**Collection reference (changed 2026-08-14):** Collection → Editions presents Base, Gray, White, Rainbow, and Violet as five runtime-size Emoji Tile samples in one unbroken horizontal row using those same live overlays. Narrow viewports scroll horizontally rather than wrapping. Hover/focus copy reuses the canonical edition name and effect; White is not shown as a letter-tile edition. Collection → Emoji Tiles counts only profile-eligible definitions as discovered; an undiscovered tile fully hides its artwork behind a neutral card and lock mark. Its tooltip reads `Not discovered / 발견되지 않음` and only advises purchasing or using it in an unseeded run; identity, effect, condition, progress, and any Record sticker remain hidden. A discovered tile with Chapter-8 mastery overlays the existing art for its highest cleared Record at the upper right; the sticker stays inside the card's tilt layer, is its own hover/focus target, and owns the Record tooltip.

**Record-sticker interaction (changed 2026-09-10):** that upper-right sticker is
its own focusable hover target and owns the Record explanation. In the overlap,
it suppresses the Emoji Tile tooltip so only the sticker tooltip is visible.

## 4. Juice spec (motion)

Priority order — implement top-down, cut from the bottom if time-boxed:

1. **Word settle sequence** (the core dopamine loop, GDD §7.1 layer 1; final-settlement display clarified 2026-09-06): whole-word stamps (suit/gibberish and the highest Word Hand) → played letter tiles from first to last, with each tile's base score, material/font/edition, and tile-triggered Emoji effects kept together → the owned Emoji Tile list from first to last → tiles still held in the hand, frozen in their visible order at Play time → consumable hooks (reserved for future mechanics) → global/boss beats → authoritative settle-complete → committed word gain and 0 × 0 ScoreBox hold for 500ms → committed word gain counts down in the always-reserved 52px status slot while the round total counts up over the existing 700ms tween. The ScoreBox axes replay ordinary word-settle beats and reset afterward. The round holds at `committedBefore` through settle and the post-signal hold, and the transfer uses only `max(0, committedScore − committedBefore)`, including flat score. Invalid, zero, negative, and non-finite gains skip both display and hold. The UI-only hold timer is fixed at every game speed, keyed by `settleId`, and cancelled on mode change or unmount; it adds no audio, shake, score event, input gate, engine state, save state, or settle duration. On a full-motion clear with a real sentence bonus, the word transfer runs alone from t500–1200 while sentence bonus remains absent and the ScoreBox stays 0 × 0; BUILD runs t1200–1900 and LAND runs t1900–2600 with the axes held at 0 × 0, and only then does the verdict beat begin. A positive no-bonus clear may publish its final score at t500 because the existing resolution wait covers the transfer; zero or non-finite gain remains immediate. Reduced Motion at or through the initial hold publishes the collapsed result at t500. The empty slot remains through BUILD so the ScoreBox does not move. Reduced Motion uses the same static 500ms post-settle hold at 0 × 0, then removes the gain while the round snaps to committed. **Play first runs a UI-only physical-impact prologue before score-event beat zero:** all submitted tiles move as one rigid `.submitted-tiles` row through anticipation, drop, contact, and recoil. Its duration is fixed at **650ms at every game speed**, independent of word length, and contact remains at 40%. One contact tick applies every local material-family burst simultaneously and fires at most one settings-scaled screen shake and one `submitThock`; it never staggers or gives the last tile a special transform. Ceramic/Porcelain/Ivory throw dry square dust, Polished/Lead Plate/Brass ring and glint, Glass produces a non-destructive angular flash, and Stone/Wood throw block/grain debris. Reduced motion keeps the existing fixed 700ms settle, removes movement/debris/shake/slam audio, and shows only a static contact highlight. The stronger Word-Hand ring appears only when the existing `letterHand` score event lands, so the prologue never reveals a hand early. The currently evaluated tile lifts slightly; every score/effect beat makes it bounce. A letter tile's own base/material/font/edition contribution uses the source tile's glow/wiggle plus a large popup above it with Chips, Mult, multiplicative factor, gold, chance, or retrigger as applicable. When an Emoji Tile effect targets a played or held letter tile, that letter tile still glows/wiggles but does **not** repeat the score tag; the single value readout appears below the firing Emoji Tile. Retrigger-only beats display localized `Again / 다시` instead of a font-dependent symbol or an empty tag. Additive Mult popups use the red Mult colour and render `+N` only; they do not append a redundant `×`. Every firing Emoji Tile shows its effect in a large popup below its owned slot: Chips reuse the blue diamond score-box treatment, additive Mult is red `+N`, multiplicative Mult is red `×N`, money uses the gold `$` readout, flat score uses the tomato symbol, and an effect without a numeric/symbolic readout displays localized `Applied / 적용`. A compound Emoji effect emits one beat per actual application: Type Orchestra fires once per distinct font including Medium, on that font's first tile in word order, each showing `×1.5`; repeated copies of one font do not add beats. **Whenever a scaling Emoji Tile's live value increases, the same owned-slot wiggle and axis-coloured numeric popup must fire; scoring growth is an ordered score-event beat, while discard/consumable/blind-end growth fires directly from the committed state change. Wastebasket uses this state-change route for its first-discard `+$2` wiggle and popup, while the central `run.gold` watcher owns the single coin sound; its blind reset is silent, and the post-blind cleanup event re-arms it after temporary boss debuffs are removed. Decreases stay silent by default, but the explicit decay roster—Dulling Pencil, Drying Ink, Folding Manuscript, and Shuriken—updates its tooltip from the owned live state and emits the matching negative popup. Each value clamps at zero; on its terminal trigger the popup completes before the owner is removed. Blind resets always stay silent.** Score-box-only feedback is insufficient. Ordinary score-event slots use **600ms / selected speed**; the submission snapshots speed only for this cadence, and settle completion uses the same calculated duration. Chance-bearing Lead Plate beats keep a **600ms real-time floor** at every speed so their result badge can appear before the next beat, with completion derived from that same timing. Enhanced Emoji Tiles keep the ordinary fixed 0.55-second popup/0.32-second wiggle and use a **1000ms / selected speed** timeline slot; the longer slot is separation between adjacent trigger/enhancement beats, not a stretched animation. Creation slots retain their 480ms floor while their source/flight VFX keep the current fixed 1× lifetime. The final tally hold stays fixed at 650ms when score beats exist; a settle-only/debuffed play remains impact-only.
...
4. **Card motion (changed 2026-07-30):** vouchers, packs, and every Fable, Constellation, and Gambler card idle with a slow 3px float plus ±0.45° sway. Pointer movement pauses the idle loop, lifts and scales the card, tilts it in 3D toward the cursor, and moves a radial sheen with the pointer; leaving eases flat and resumes the idle phase. Keyboard focus straightens, lifts, and adds a gold outline. Generic select = rise 10px + gold ring; shop-offer select raises the complete product/price/action layer by 59px (15px base + 44px action height). Collection pack entries keep the grid image-only—no persistent pack-type, grade, or coming-soon label—but restore the shared type/description/grade tooltip on hover or keyboard focus.
   ...
   **3.5 seconds** and uses the same body-portalled modal focus/input contract as
   the shared consumable result vignette.
...
7. **Score Keyboard / 점수 키보드 (typing speed changed 2026-09-08):** a non-interactive fictional vintage terminal keyboard is fixed in the left viewport margin. The local **1774×887 transparent RGBA pixel-art master** is a natural 2:1 landscape object with three empty flat key wells and no baked caps or legends. Raster and DOM keys share one wrapper rotated 90° counter-clockwise into a vertical 1:2 dock. Width is `min(230px, 13vw, gutter − 16px, (100vh − 32px) / 2)`, height is `width × 2`, the dock is vertically centred, and `object-fit: contain` preserves the art without crop, stretch, filter, or active-opacity changes. A UI-only formula/row registry places **101 fully opaque labelled DOM caps** across the main, navigation, and numpad wells: ESC, F1–F12, PRINT/SCROLL/BREAK, the standard no-Windows-key main block, ten navigation/arrow keys, and 17 numpad keys. A–Z appears once; Backspace/Tab/Caps/Enter/shifts/Space/numpad 0/+ /Enter keep their authentic 2u/1.5u/1.75u/2.25u/2.25–2.75u/7u/2u proportions. Every cap remains disabled, untabbable, accessibility-hidden, fully opaque, and part of the deterministic candidate pool. Each `settleId` resets to Tier 0. Only a positive current-submission score beat activates classification. Compute `local = max(0, Chips × Mult + flat score)`, add the settle-id-snapshotted visible `sentenceAssist = max(0, projectedScore − committedScore)` (zero under `previewHidden`), divide by the positive finite target, and classify Idle below 1× and Tiers 1–5 at `[1, 1.2, 1.3, 1.4, 1.5]`; present `max(previous peak, raw tier)`. Decrease/zero/gold/stat/invalid beats and sentence assist alone neither demote nor promote it; committed/earlier scores and later sentence BUILD/LAND never enter directly. The current beat's peak drives one semantic primary before deterministic fill: submitted letter (or Stone→Space), Enter for suit/length/Word Hand/global, stable F1–F12 for untargeted Emoji Tiles, Break for bosses, Tab for Tags, Space for Pouches, and Enter fallback. Tiers use 5/10/12/16/20 visible and 3/4/5/6/7 audible presses; the primary sounds first. Each audible slot resolves the same visual key and uses a locally bundled 32-WAV pool: registry index i maps to sample (i mod 32) + 1, while an unknown id uses main Enter. Samples load only on play and fall back to the matching existing synthesized recipe on failure. Normal settle completion returns idle. An actual clear with a non-zero peak, including a sentence-assisted peak following a positive local beat, starts cycle 0 immediately after settle completion, then repeats the existing score-keyboard renderer through BUILD/LAND, final count-up, verdict, and the live Fee Settlement row/total animation. Synthetic ids are `clear:{blindKey}:${settleId}:${cycle}`; main Enter is always primary and its first audible slot uses Enter's mapped WAV at the existing `deskEnter` accent gain, while deterministic fill uses each visible key's mapped WAV at the ordinary `deskKeycap` gain to complete the tier count. Repeat intervals use the 1.5×-faster fixed `418ms / 1.5` (≈279ms) beat × `[2, 1.5, 1.25, 1, 1]` for Tiers 1–5, independent of selected speed; active Tier 1–5 key motion/audio uses the same faster cadence and a visible 4px/.86 physical depression, while the target cue retains its current timing. Each cycle replays every selected key's local body/key motion (Tier 1 shake factor 0.1), Tier-3 smoke, and Tiers-4–5 smoke/flame/POP; Held tiers retain their 1.5×-faster LED/smoke cadence, while the former gold chassis border and glow are removed. Collect's transition to Shop, terminal modal, next blind, unmount, or Reduced Motion ON cancels current/future cycle work synchronously. Temporary settle/tile/Emoji VFX still finish on their ordinary timelines. Loss, Tier 0, a true local-zero sentence-only clear, and non-clear paths never loop. The target-cross event remains one logical once-per-blind Enter strike; celebration cycles intentionally reuse Enter's mapped WAV at the `deskEnter` accent gain without retriggering that event. Reduced Motion keeps a static peak/Enter accent and the existing target-cross audio exception, with no repeat audio or moving VFX. The portal retains explicit `world-mono` grayscale until colour unlock, and neither the hold nor active state changes raster/key opacity or filters. No cycle affects global shake or settle timing.
   ...
   **Lv.N · zodiac symbol · pattern name** callout using the existing 1.7-second
   ...
   supplemental row keeps score/controls at 188/296px; two rows use 211/273px;
   three rows use 234/250px. Each pair totals 484px; the always-reserved 52px status/transfer slot stays empty during BUILD, and a visible internal gap separates the ScoreBox from the first supplemental row without changing those fixed heights.

> **Will Score Keyboard exception (changed 2026-09-09):** keep Will's final Chips ×0.5 and Mult ×0.5 engine beat and the ordinary score-event order. During a Will submission, suppress every intermediate Score Keyboard beat, then start one Break-primary beat from the final post-Will axes at the beginning of the existing 650ms final tally hold. Reduced Motion uses the same final-axis tier. This adds no timer and does not change `settleDurationMs()`.
8. **Side interactions** (ambient, non-gameplay; right-only changed 2026-08-27): the random encounter pool contains a linked pixel-art coffee cup/pot pair, hotel-style call bell, blank cheque, two tactile toys—**Wax Ball / 왁뿌볼** and a standalone mechanical **Keycap / 키캡**—**Shaco's Surprise Box / 샤코의 깜짝 상자**, a hovering **Flying Pest / 날벌레**, **Bulldog Roulette / 불독 룰렛**, and a covered **Nuclear Launch Button / 핵 발사 버튼**. Each is set down from above. Most leave after one completed interaction; Bulldog Roulette remains until its winning tooth is found, and the launch button requires two deliberate clicks. Coffee state starts full, so only the filled cup is eligible; three staggered pixel-steam wisps rise while it waits, and clicking drains its separately layered coffee with a sustained water-draining rush, keeps it empty throughout the exit, then removes it. Only after that drink may the coffee pot roll. Clicking the pot plays a continuous water-pouring stream with small splashes, restores the shared coffee state as it exits, and makes the filled cup eligible again while making the pot ineligible. Clicking the call bell visibly pushes its independently layered top switch down before it springs back, shakes the bell body, emits stepped side-ring marks with a natural struck-metal resonance, then the bell exits. Wax Ball cracks/pops with a loud layered dry snack-like crunch; the complete, unclipped standalone Keycap depresses with an audible mechanical down/up click and emits stepped impact marks before each exits. Clicking Shaco's Surprise Box replaces its closed frame with an opened frame whose spring-mounted clown doll overshoots upward with a synthesized boing before the encounter exits. The Flying Pest jitters in place until clicked; a layered fly swatter then slaps diagonally across it with a sharp impact burst, and the insect spins downward out of view. Bulldog Roulette exposes eight individually clickable teeth and seeds one winning tooth on spawn. A safe tooth depresses with a plastic click and remains pressed without dismissing the encounter; the winning tooth snaps the bulldog's mouth shut with a bite sound, then sends it out. The Nuclear Launch Button arrives as a square bolted metal panel with its oversized red button behind a cyan glass safety cover. The first click flips the cover open with a glass-and-metal clack; only the second click depresses the button, flashes its warning lights, plays a rising alarm and low launch impact, then sends the panel out. The blank cheque may be **signed by dragging anywhere across the whole cheque object** (mouse or touch): its quantized SVG stroke and pixel pen follow the pointer while short paper-scratch bursts track actual drag movement. A small, low-contrast localized “Sign here” hint rests above the signature line and disappears as soon as drawing begins. A click or tiny accidental mark is rejected and cleared; only a deliberate stroke completes the signature, after which the cheque exits. During a run, the safe right-gutter footprint may shrink objects below their full responsive master ranges (ordinary objects 112–168px; landscape cheque 144–260px)—down to 68px for ordinary objects at 960px—rather than overlap the board. The **right margin alone** owns three non-overlapping vertical zones, ordered oldest-first from the bottom and compacted when an object exits; its lowest zone starts at least 136px above the viewport bottom (or 14vh, whichever is larger), clearing the complete tile-pouch dock. The left margin is reserved for the persistent Score Keyboard. **Purely cosmetic and never required**: encounters grant nothing, never overlap the interactive board, and never punish being ignored. At most one generic encounter (pot, cheque, tactile toy, surprise box, Flying Pest, Bulldog Roulette, or Nuclear Launch Button) is active alongside the independent cup/bell slots; overall encounter frequency uses a randomized **70–140 seconds** between rolls.
   **Coffee aperture (changed 2026-09-04):** a CSS-rendered empty aperture at `left 20% / top 21% / width 54% / height 15%` covers the unchanged raster's baked brown surface. The separate liquid sits inset at `left 22% / top 23% / width 50% / height 12%` with no independent border; the layer order is raster → cavity → liquid → steam. Clicking keeps the existing drain/exit lifecycle; either Reduced Motion path shows the cavity empty immediately.
   **Rotating Laboratory screen (DEV-only; changed 2026-09-07):** Main Menu → Laboratory keeps the shared 1440×988 shell, CRT treatment, Back, and Escape, but its interior is a single replaceable review surface rather than a gallery. Only a completed player-facing implementation or change that cannot be inspected immediately in Collection or Run Info qualifies; internal-only refactors do not. The next qualifying change replaces the prior contents, while automatic tests retain old preview regressions. Always reuse the real production component/data and never add engine, RNG, run, discovery, profile, settings, or storage state. The current surface reuses production `ScoreTypewriter` with deterministic UI-only Tier 1–5 controls and Replay so the updated key counts, 1.5×-faster typing cadence, physical depression, LEDs, smoke, and flames can all be checked while resizing the viewport. The former coffee-cup, money-ledger, money-total-motion, hidden-pattern, and word-score-transfer previews remain covered by automatic regressions.
...
- **Placement (full-label wrapping changed 2026-09-09):** bottom-aligned below the Next Blind/Reroll controls in the existing 168px shop rail (proprietor behind the counter feel), with no separate local balance panel and never overlapping the item slots. Both controls remain equal-sized and wrap overlong localized copy in full with reduced horizontal padding instead of widening the rail or showing an ellipsis. Hidden on the ≤720px single-column layout, lying on a pink **cat-face plush cushion** (`docs/Arts/Mascots/T_Cushion.png` → `src/ui/assets/piyak-cushion.png`) — the plush sits behind him, ears peeking above his back. (The earlier in-house generated pillow is retired.)
...
- **Role in shop (wrapping changed 2026-09-06):** on each shop entry Piyak shows one random line from the `voice.piyak.welcome.*` pool (8 lines, i18n, resolved through `voicedKeys`) in a pixel-grammar speech bubble (squared corners, ink border, blocky shadow). Shared mascot, tutorial, and guided-intro dialogue uses at least `--fs-lg`; bubbles size to their copy inside the existing surface cap, keep words intact where possible, and use the shared last-resort wrapping fallback without balanced wrapping. Purchase/reroll reactions remain a later layer. Track in screens §2.6.
...
**Score Keyboard jackpot/smoke follow-up (changed 2026-09-03):** the red/yellow/green indicators flash in the deterministic random-looking order with rapid hard-edged jackpot pulses. Align each light's centre with the corresponding raster indicator and spread the light sideways with pixel-hard rays. Tier 3–5 pressed caps emit visible smoke and Tier 4–5 caps emit larger visible flames, using per-key 0.95–1.05 variation; Tier 5 keeps ×1.35 smoke and ×1.25 flame base multipliers. Seven small chassis-smoke sources appear at Tier 5, while Tier 5 uses all twelve at larger size and dark-gray tone. Do not use one large smoke overlay.
