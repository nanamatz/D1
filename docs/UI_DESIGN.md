...
bottom. Overlays such as Fee Settlement and Options are the exception: they centre
...
5. **Boss intro (changed 2026-07-28):** only actual entry into a `playing` Deadline board triggers the centred, non-blocking card—Blind Select never does, while Continue into an active saved Deadline does. It shows the boss emblem, localized name, and full debuff text, completes its overshooting stamp/pop, holds for 0.5 seconds, then leaves with a short lift/fade. Reduced motion removes both animations. The earlier badge-only stamp is retired.
