...
| `ScoreBox` | blue Chips box × red Mult box, Jersey 10 numerals, count-up animation on settle. The always-reserved 52px status slot immediately above it temporarily shows the authoritative positive word gain with no plus sign after full-motion settle completion; that neutral readout uses the round score's `var(--readout-size)` with line-height 1, decreases to zero as the committed-only round total increases over the existing 700ms tween, and leaves an empty slot through BUILD so the ScoreBox never moves. Ordinary status copy remains single-line with ellipsis. The transfer uses `formatScore`, is decorative/`aria-hidden`, and never shows zero, negative, or non-finite values. Reduced Motion holds the full gain in the same slot during the existing reduced settle, then removes it as the round total snaps to committed. Both axes use shared `formatScore`: round once to integers; comma-format below 1,000; from 1,000 use the largest `K/M/B/T/Qa/Qi` unit with at most one decimal truncated toward zero; retain compact scientific notation from `1e21`; and show an em dash for non-finite values. Each formatted string is reused once and independently classed by visible length: ≤7 characters uses 52px, 8–9 uses 38px, and ≥10 uses 30px. The inner numeral is centred, single-line, and clipped to its box while the outer box stays overflow-visible for contribution VFX. Accessible labels announce localized Chips and Mult with those exact formatted values. The actual axes are never redistributed. Semantic effect popups keep their own axis precision. |
