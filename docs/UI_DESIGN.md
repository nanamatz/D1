...
**Other feature-02 D visuals.** *D-1 joker reorder:* the owned-joker shelf is drag-reorderable and **order = hook execution order** (GDD §11 intro). *D-5 tomato score icon:* the icon beside score numbers (blind-badge target, round score) is a **pixel tomato** (`src/ui/assets/tomato.png`, from `docs/T_Tomato.png`; tomatoes thrown at bad manuscripts) — the term "Chips" and the blue chips box are unchanged. The tomato is greyscaled until **RED** unlocks (it belongs to the red group). *D-6 stage backgrounds:* per-stage backdrops under the CRT pass — **초고/Draft** = writer's desk, **퇴고/Revision** = marked-up manuscript, **마감/Deadline** = the red-pen office (tense red wash); desaturation-aware (GREEN unlock, C-3, will later gate their color). *D-7 (changed 2026-07-27):* Collection uses a centred framed modal. Its root menu is an asymmetric two-column stack whose category-button thickness is proportional to a normalized estimate of that category's visible row footprint; both columns use the same total weight so equal weights stay physically equal. Fable, Constellation, and Ink Cards form one inset three-button family block and use distinct purple/cyan/green buttons. Fable uses 18 high-detail, path-only SVG illustrations normalized to the same `500×700` 5:7 canvas (`250×350` logical pixel grid, 32 colors); every illustration is stretched to the full common image bounds established by The North Wind and the Sun, giving all cards identical visible width and height without cropping. Each traced illustration retains its original English title plate, while localized names remain available in tooltips and accessible labels. The 5-column, 10-per-page gallery and the shop, pack, and held-card slots reuse the same shared framed component. Constellation renders its 12 supplied images in a 6×2 gallery; Ink retains the uniform `?` scaffold until its registry arrives. Detail modals have no shared fixed minimum height and instead size to their actual grid rows. The orange Back bar spans the modal footer; mobile collapses the menu to one column. Where noted, icon/background art currently ships as an emoji/CSS placeholder pending the pixel-art pass.
