...
| `VoucherCard` | **One shared CSS-rendered 3:2 ticket** in the shop, Collection, and Run Info (`138×92px` at the base UI scale). The approved 2026-07-26 ticket reference defines the silhouette/style; the centered icon uses the matching PNG from `docs/Arts/Voucher/` for each of the 32 vouchers (including `StoryBook.png`). Stepped side notches, warm paper/dither, heavy pixel frame, vertical VOUCHER marks, and a black inset nameplate remain identical. Price and purchase controls sit outside the ticket so its width, height, ratio, and internal layout never vary. Every voucher uses the shared card idle float and cursor-following 3D tilt/sheen. **Collection layout (`docs/reference.png`, changed 2026-07-27):** four base→upgrade pairs per page in a 2×2 pair grid, four pages total. Locked upgrades render as muted `?` tickets named **Undiscovered / 발견되지 않음**. Their tooltip contains only “Redeem this voucher in an unseeded run to discover what it does.” / “시드되지 않은 런에서 이 바우처를 교환하여 기능을 알아보세요.” The real name, effect, unlock condition, and progress are not exposed until unlocked. |
| `ShopOffer` | **Image-first sale slot (changed 2026-07-27, `docs/Arts/refer.png`).** Every sale card reserves the same proportion-preserving `144×185px` transparent stage, but the actual product art, price, and contextual action live together in a separate foreground layer sized to the art's own aspect ratio. Pack foregrounds use 88% of the stage height and sit 12px above the common centerline so their tall wrapper art remains inside the pack stall at maximum tilt; other families retain their family-specific fit. The voucher/pack background panels keep a `273px` minimum height—5px taller than the preceding layout—even after the cards shrink. The stage has no visible background or sheen. Every ancestor keeps overflow visible, so idle sway, cursor tilt, lift, and scale can never crop the product. The price badge sits 23px above the foreground image's top edge and the selected **Buy / 구매**, **Redeem / 교환**, or **Open / 열기** button is absolutely attached beneath its bottom edge; neither consumes shop layout space, and both move with the image. A selected offer raises the entire foreground layer above neighboring stalls. Names and classifications remain available through the shared tooltip. |
| `HeldConsumable` | **Object-layer shelf rendering (changed 2026-07-27).** The `84×112px` held slot is only a transparent layout reservation. Supplied Fable/Constellation art renders directly as an `80×112px` foreground object—no beige wrapper card, inset image box, persistent name, crop, or rectangular sheen. The foreground itself owns idle motion, cursor tilt, keyboard focus, click selection, and the shared tooltip. Opening it raises the object above the shelf and attaches the Sell/Use action layer absolutely beneath the image instead of reflowing or sitting beside the shelf. Empty slots keep their dashed placeholder. |
...
**① Every material gets a shape/texture cue, not just a hue.** Texture must survive desaturation (the game may be running fully monochrome, §13).
...
|---|---|---|
| Porcelain (base) | plain warm face | — |
| Ceramic | glazed sheen, subtle speckle | — (flat +Chips) |
| Polished | animated pixel shine sweep | — (flat +Mult) |
| Glass | translucent + inner glow, faint crack hairlines | **crack/risk mark** (1/4 destroy) |
| Stone | grey speckle/dither, **no letter** (already unique) | — |
| Lead plate | dark type-metal, beveled edge | **dice pip** (Lucky rolls) |
| Ivory | cream with sheen | **$** (pays at blind end *if held*) |
| Brass | metallic gold dither + glint | **hand icon** (×1.5 *while held*) |
| Wood | grain pattern | **live growth counter** — the tile's current +Chips, since it permanently grows +10 per play; a Wood tile that has grown must *look* grown |
...
6. **Tomato reactions** (the score icon, D-5): the tomato gives a compact, springy **bounce on every scoring beat** — it is the thing that visibly reacts while chips count up, so the board stops feeling static. Between beats it occasionally **hops** on a long random idle timer (a few times per blind, never rhythmic enough to distract). Its panel-relative anchor is a separate, non-animated layout layer: score and idle motion always begin at the displayed tomato position, stay within roughly 4px vertically, and never replace the anchor transform or jump across the panel.
7. **Burning score boxes** (Balatro port): while the settling total is **at or above the blind target**, the chips and mult boxes ignite; **flame size scales with the size of the overshoot**. This is not decoration — with auto-settle (GDD §7.2) there is no cash-out button, so the flame *is* the "you've cleared it" signal, arriving a beat before the settlement resolves. It must ignite during the count-up, not after.
8. **Side interactions** (ambient, non-gameplay): occasional desk objects enter the margins of the play screen — the large pixel-art coffee cup is set down from above, a pencil rolls, and a paper plane crosses. Clicking one plays a small animation + SFX. The cup keeps its position while its separately layered coffee drains in stepped pixel increments with a distinct *slurp*, then exits; the cup art is responsive between 112–168px. **Purely cosmetic and never required**: they grant nothing, so no player ever feels punished for ignoring them, and they must never sit where a mis-click could hit the board. Random encounter timing, at most one on screen at a time.
9. **Drag dot-outline (feature-02 D-2)**: while dragging a tile or a joker, the origin slot shows a dashed pixel outline and the live insertion gap shows a dashed bar (Balatro feel). Applies to hand, tile zone, and the joker shelf.
