...
const locales = ['en', 'ko', 'ja', 'zh-CN', 'zh-TW', 'pt-BR', 'de', 'es-ES', 'fr-FR'].map((lang) => JSON.parse(
...
describe('Settings tooltip coverage', () => {
  ...
  it('accounts for every one of the 20 native Settings focus targets', () => {
    ...
    for (const locale of locales) {
      expect(locale['settings.audioNote']).toMatch(/^(Audio|오디오|オーディオ|音频|音訊|Áudio)$/);
      expect(locale['settings.mute']).toMatch(/^(Mute|음소거|ミュート|静音|靜音|Silenciar|Stumm|Muet)$/);
    }
