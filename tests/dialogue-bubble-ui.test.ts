...
describe('dialogue bubble CSS contract', () => {
  ...
  it('content-sizes bubbles and safely contains translated copy', () => {
    expect(css).toMatch(/\.mascot-bubble\s*\{[^}]*box-sizing:\s*border-box;[^}]*width:\s*max-content;[^}]*max-width:\s*100%;/s);
    expect(css).toMatch(/\.mascot-bubble\s*\{[^}]*text-wrap:\s*wrap;[^}]*word-break:\s*keep-all;[^}]*overflow-wrap:\s*anywhere;[^}]*hyphens:\s*none;/s);
